...
# 9. AT-321-C contact audit bypass gate
#
...
if ($SkipPipelineGate) {
    Write-Host ''
    Write-Host '9. Contact audit bypass gate (AT-321-C) -- skipped (-SkipPipelineGate)' -ForegroundColor DarkGray
} else {
    Write-Host ''
    Write-Host '9. Contact audit bypass gate (AT-321-C)' -ForegroundColor Yellow
    ...
    if ($rawWritesC.Count -gt 0) {
        $auditTestsC = $changedNorm | Where-Object {
