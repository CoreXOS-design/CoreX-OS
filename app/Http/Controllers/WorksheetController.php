...
class WorksheetController extends Controller
{
    public function index()
    {
        ...
        // ---- Branch market averages (fallback when agent has no deals) ----
        $branchMarket = null;
        if ($user->branch_id && $worksheets->first()) {
            $branchMarket = \App\Models\Deal::marketAveragesForBranch(
                (int)$user->branch_id,
                (string)$periodForStats,
                ['pending','granted','registered']
            );
        }
...
// ideal comparison (ex VAT)
    ...
    public static function calculateWithOverrides(Worksheet $w, float $avgSalePriceOverride, float $commissionPercentOverride, bool $commissionIsExVat = false): array
    {
        ...
        // Commission per sale: if the caller already passed an ex-VAT percentage
        // (e.g. effective_commission_percent_ex_vat from dealRegisterStats), skip VAT removal.
        $grossCommissionPerSale = $avgSalePrice * ($commissionPercent / 100);
        $commissionPerSale = $commissionIsExVat
            ? $grossCommissionPerSale
            : ($grossCommissionPerSale / (1 + $vatRate));
