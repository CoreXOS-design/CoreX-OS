...
class CommercialEvaluationPdfService
{
    ...
    public function generate(CommercialEvaluation $evaluation): string
    {
        ...
        $branchName = $esc($evaluation->branch->name ?? 'Agency');
        // White-label: disclaimer/footer read the evaluation's own agency, never HFC.
        $companyName = $esc(optional(\App\Models\Agency::find($evaluation->agency_id))->name ?? ($evaluation->branch->name ?? 'Agency'));
        ...
        // Disclaimer
        $html .= $this->disclaimer($companyName);
...
HTML;
    ...
    private function disclaimer(string $companyName): string
    {
        return <<<HTML
...
<div class="disclaimer">
    ...
    {$companyName} and its agents accept no liability for decisions made based on this evaluation report.
</div>

<div style="text-align:center;margin-top:24px;padding-top:16px;border-top:1px solid #e2e8f0;">
    <p style="font-size:10px;color:#94a3b8;">{$companyName}</p>
    <p style="font-size:9px;color:#cbd5e1;">This report was generated by CoreX OS</p>
</div>
HTML;
    }
}
