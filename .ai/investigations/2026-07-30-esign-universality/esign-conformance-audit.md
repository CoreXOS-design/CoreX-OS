...
| **R3** | `SignatureService::handlePartyCompletion:1169`, `advanceToNextParty:1567`, `holdForFinalAgentReview:1658`, `approveAndAdvance:1353`, `completeDocument:1906`, `isFullyComplete:1139`. File+email inside `completeDocument`, reached only via `approveAndAdvance` | **U** | **UNIVERSAL auto-advance CONFIRMED 2026-07-30** (Johan): clean completion → next recipient with NO between-recipient agent gate; change/flag → mid-flow `PENDING_AGENT_APPROVAL`; final clean → `holdForFinalAgentReview` (no auto-file). Both `completeWeb()` and `complete()` now delegate to `handlePartyCompletion` — the marker path's residual between-co-owner gate was removed. Flow spec: `esign-recipient-signing-fix.md` "UNIVERSAL SIGNING FLOW". **Wet-ink carve-out** `SignatureService.php:1256,1258,2493`. |
