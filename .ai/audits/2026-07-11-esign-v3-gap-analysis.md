...
|---|--------|----------------|
| **H1** | **There are TWO amendment paths, and the legacy one voids signatures.** `requeueAllPartiesForInitialing()` is the good one (retains marks, re-initial only). But `SignatureService::handleAmendment()` still exists and sets previous signers back to `STATUS_PENDING` with a fresh token — **a full re-sign cascade**. ~~`AmendmentController` only calls the initialing path, so the doctrine holds *today*~~ **⚠️ CORRECTION (2026-07-11, during the P0-6 build): THIS WAS WRONG, AND IT MATTERED.** A dependency trace found `handleAmendment()` is called from **two live paths in `SigningController`** (web signing and PDF signing — the "Other Conditions" amendment-detection flow). It was **reachable in production**: a recipient typing a condition could **un-sign every party who signed before them**. Not a dormant landmine — a live defect. **Fixed in P0-6** (`7d5dc851`): the proposal now halts the flow and hands it to the agent; prior signers are untouched and are re-circulated — with their marks retained — only on the agent's approval. *Lesson: "no caller" is not a conclusion until you have grepped every caller, not just the one you expected.* |
...
| 41 | **Completion distribution — signed copies go back to the signers** (ceremony **§11-B**, added 2026-07-11) | **EXISTS-AS-IS** (trigger, recipients, scope) + **EXTEND** (as-many) | **Trigger — already correct.** Candidate doctrine holds today: a candidate's sign-off calls `advanceToSupervisor()` and completes nothing; only `supervisor_final` calls `completeDocument()`, which fires `sendCompletionEmails()`. Authoriser recorded + audited. **Recipients — already correct:** completed requests only, `agent` skipped. **Scope — correct by omission:** only the signed PDF is attached; pack attachments are never touched — ⚠️ **must become an explicit rule** before distribution iterates the pack. **The one real gap — HOW:** one merged `"Signed - {documentName}.pdf"` today; a four-document pack must distribute as **four attachments**. **The per-document PDFs already exist** — `filePackDocuments()` writes each to `docuperfect/signed-documents/{sigTemplateId}/individual/{templateId}_client.pdf`; they are simply not the ones attached. Also make `ACTION_SIGNED_PDF_EMAILED` **per-document per-recipient** for the evidence timeline (§6). | **S–M** |
