...
- **Confirmed display-only, checked everywhere it could plausibly surface:** the real Payroll module (payslips, payroll runs) does not read this ledger anywhere. Neither does anything under the API routes (nothing reaches a mobile app or any external integration), and no automated email/notification carries this figure. So this wrong number is not paying anyone the wrong amount, and it isn't leaving the app through any channel I could find.
...
- **This same wrong figure surfaces on TWO agent-facing screens, not one:** the dedicated "My Earnings" page, and — more significantly — the "this month" / "this year" earnings widgets on an agent's own main Portal page (`/my-portal`, their everyday home screen, not somewhere they'd have to specifically navigate to). Both are gated only by "not an assistant" — **any agent, branch manager, admin, owner, or super_admin** can see their own wrong figure there, regardless of the owner/super_admin-only gate on the company-wide Overview/Management screens.
- **One root cause, not two:** the function that applies the wrong (generic, agency-wide) split percentage instead of the deal's own agreed split was written back on 2026-03-27, as part of the Commission Engine's original build — and had exactly zero callers anywhere in the codebase until 2026-08-19, when a real, deliberate fix (`2d402a92`, written by Johan himself, closing a genuine reported bug — "Retha's commission generation didn't work") gave it its first and only caller. That caller hands it an amount already split by the deal's own specific percentage; the five-month-old function was never built or tested to receive an already-split figure. The "double split" and "uses the wrong default percentage" are the same single mismatch described two ways, not two separate defects — and it was never exercised against a real deal before Aug 19, because nothing had ever called it before then.
