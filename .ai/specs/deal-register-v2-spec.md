...
## 20. AT-406 — Deal Register list: Actions column ran off-screen on an ordinary laptop

`resources/views/dr2/index.blade.php`. Presentational only — no controller/route/model/DB change.

- **Root cause.** The table had no `table-layout: fixed`, so its `<colgroup>` widths were advisory hints, not real constraints — a browser under default `table-layout: auto` grows a column past its stated width whenever content can't fit. The Actions cell held up to 4 unshrinkable links (Log, Pipeline/Attach, Edit, Pay) in a `flex` row with no wrap. Net effect: the table needed ~1425px against a ~1076px content pane on a 1366px-wide laptop, and Edit/Pay rendered off-screen to the right. Same defect pattern confirmed present on DR1's equivalent screen (`resources/views/admin/deals/index.blade.php`) — reported as a bug class, not fixed here (out of this ticket's scope).
