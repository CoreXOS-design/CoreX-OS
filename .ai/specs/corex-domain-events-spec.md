...
| `Contact\ContactMarkedLostInBuyerPipeline` | A Contact's `buyer_state` transitions TO `lost` (auto-recompute or manual override) | `contact`, `triggeredByUserId`, `agencyId` | `BuyerStateService::transitionTo()`, dispatched explicitly (its `updateQuietly()` call suppresses model events, so an observer never fires) | `SetAsideCoreMatchesOnBuyerLost` |
| `Contact\ContactRestoredFromLostInBuyerPipeline` | A Contact's `buyer_state` transitions AWAY FROM `lost` | `contact`, `triggeredByUserId`, `agencyId` | `BuyerStateService::transitionTo()`, same explicit-dispatch reason as above | `RestoreCoreMatchesOnBuyerRestored` |
...
| `Agent\AgentBranchAssigned` | An agent's home branch is set or changed | `user`, `branch` (new), `actorUserId`, `reason` (`manual` \| `branch_archived`), `fromBranchId` | `UserManagementController` (manual), `BranchAssignmentController::deleteBranch()` (archive wizard, one per moved agent, inside the archive transaction) | `Agent\LogAgentEvent`, audit. The dated `user_branch_history` row is written by `UserObserver`, not by a listener. Spec: `.ai/specs/branch-archive-reassignment.md` §8 (AT-420) |
| `Branch\BranchArchived` | A branch is archived (soft-deleted) after every attached person was moved | `branch` (already trashed — read with `withTrashed()`), `actorUserId`, `movedUserIds[]` | `BranchAssignmentController::deleteBranch()`, inside the transaction, after the soft delete | `Agent\LogAgentEvent`, audit. Spec: `.ai/specs/branch-archive-reassignment.md` §8 |
| `Branch\BranchRestored` | An archived branch is restored to active (nobody is moved) | `branch`, `actorUserId` | `BranchAssignmentController::restoreBranch()` | `Agent\LogAgentEvent`, audit. Spec: `.ai/specs/branch-archive-reassignment.md` §8 |
...
| `AgencyFeatureToggled` | An agency turns a registry feature on or off (actual state change only, never a no-op save) | `agencyId`, `featureKey`, `enabled`, `changedByUserId` (**scalars only** — no models) | `FeatureSettingsController@update` (Phase 5) | (sync) cache-bust of `AgencyFeatureService`'s per-request memo; audit. **Sync + scalar** — a queued listener on a domain event fatals (readonly `$eventId`); heavy work dispatches a Job. Spec: `.ai/specs/corex-feature-registry.md` §3.8 |
...
4. **The Webinar events have NO listeners, deliberately.** All three mails are queued directly by the service, the command and the controller, because two of them carry a plaintext access code whose lifetime is the transaction that mints it — routing a live credential through an extra hop buys nothing. The events exist so a registration lands in `domain_event_log` like every other fact, and so anything that later needs to react to a signup has a named contract instead of its own query path. Any listener added later must be registered **explicitly** in `AppServiceProvider::boot()` (auto-discovery is off) and stay **synchronous**, queueing a Mailable rather than itself.

   `WebinarJoinLinkSent` adds one constraint the other two do not have: **it is dispatched inside an open transaction** (spec `webinar-registration.md` §4.4 — the join-link send is all-or-nothing, so its stamps, its queued mails and its events roll back together). A listener on it therefore must not assume the registration is committed, and must not perform any side effect that cannot itself be rolled back. If one is ever needed, hang it off `DB::afterCommit()` rather than the event.
