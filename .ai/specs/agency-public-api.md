...
**ListingResource is P24-parity for marketing data, MINUS exact location** — it carries the rich marketing field set CoreX also syndicates to Property24: `costs` (rates/levy/special levy), a `rental` block (lease period, deposit, rental amount, gross/net, per-period rates) populated only for rentals, `mandate_type`, `pet_friendly`, `spaces`, `features`, a categorised `gallery`, `video` (YouTube/Matterport/virtual tour), and upcoming `show_days` — alongside the core price/beds/baths/size/images/agent. Numeric fields are coerced to int/float for a clean contract.

**Location is SUBURB-LEVEL ONLY (privacy guarantee, 2026-06-29).** The public listing payload exposes location no more granular than the suburb: `suburb`, `town`, `city`, `province`. It NEVER exposes the street address (`address`, `street_number`, `street_name`), the in-building location (`complex_name`, `unit_number`, `floor_number`, `stand_number`), or GPS coordinates (`latitude`, `longitude`). A property's exact location is seller PII — unlike Property24 (a paid portal under contract), arbitrary agency websites do not receive it. The cut is enforced once, in `ListingResource`, so it holds identically for the pull API (`GET /api/v1/website/listings`) and the webhook payload (`DispatchAgencyWebhooks` resolves the same resource). Internal CoreX retains the full address; only the public shape is reduced. To refresh sites that cached an older payload, run `php artisan website:rescrub-listings` (fires a `listing.updated` webhook for every live listing; PULL-based sites need no action).
