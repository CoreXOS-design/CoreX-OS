...
- Not yet done: a real-browser pass on the PUBLIC applicant-facing form specifically (token-gated, needs a live send-and-open cycle rather than a direct agent login) — the component/validation is shared with the agent form already proven above, and the same field markup was added in the same edit, but the public route itself hasn't been clicked through in this pass.
