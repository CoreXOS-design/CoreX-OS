...
- Self-approval block (an authoriser deciding on an application they themselves created) — **proven live**, follow-up pass: a throwaway RO whose OWN `created_by_user_id` matches the application attempted to approve it — `403`, "You created this application, so it needs another authoriser." Confirmed the UI itself (not just the API) surfaces this correctly too: the Decision panel replaces Approve/Decline/Request-more-info entirely with a plain-language explanation ("Only a CO (Override) user or an administrator may approve, decline, or request more information on an application they created themselves. Ask another authoriser to act on this one.") — not a raw error the agent has to interpret.
...
## Round 5 — reclaim screen space for the PDF (2026-09-11, cc3)

Johan, real browser, 1522px viewport, in one sitting: "theres no ways anyone can read that. panels are too large and the pdf showing is too small... left marker panel first... cut that panel at least into half its size... top to bottom we are only using half the screen." Then, same sitting: "cut it down to 1 row" (the header) and "the savings moves the moving bar / splitter back to the right" (the affordability panel). All three landed together — same job, stop the chrome eating the screen.

**1. Left mark-up tools panel — 190px → 90px.** Measured before touching anything: content ran from the panel's own label to the Undo/Redo row, then EMPTY for roughly half the column's height — "be clever about it," Johan's own words, meant reworking the shape, not shrinking the old one. The old shape (a full-width labelled row per highlighter, per stroke size) was what forced the width, not the tool count. Highlighters are now a wrapping grid of colour swatches — colour is already the identity, the label moves to a `:title` tooltip (the same degrade-legibly pattern used elsewhere on this screen) — scaling to however many an agency configures (2026-09-09: "an agency can have 10 highlighters"), never hardcoded to 3. Stroke buttons draw the actual line at that stroke's own thickness instead of showing "Thin"/"Medium"/"Thick" text that never fit 3-across this narrow. Undo/Redo become an icon pair. The active-state tick and the currently-selected stroke both survive, restyled to fit rather than dropped.

**2. Header — three stacked rows → one, and a real duplication bug fixed, not just squashed.** Johan quoted the exact defect: "No property linked" appeared on two consecutive lines — the page's plain subtitle (`property_address_override ?? property->address ?? fallback`, never considered the property's own `title`) and the locked-state property widget (`property->title ?: buildDisplayAddress()`, never considered `property_address_override`) were two independent, incomplete computations that happened to agree by luck whenever neither had anything to show. One `$propertyLabel` computation now, folding in everything either version knew, used everywhere on the header. The header is a single flex row: bold title truncates on its own (a genuinely unbounded contact name is the only unbounded content here), the property/lock/submitted-for-approval facts are `flex-shrink-0` so they never clip, and the whole row carries a `title` attribute with the full untruncated text — truncate-with-tooltip chosen deliberately over letting the row wrap to a second line, which would just recreate the "lots of wasted screen" complaint one row lower. For the agent's unlocked (editable) case, the property-link widget's own duplicate fact text was also removed — only its buttons remain, appended inline onto the same row.

**3. Right-hand affordability panel default — 460px → 400px.** Three measured reductions, not a guess-and-shrink:
   - `.corex-input`'s padding was never set by this file at all — confirmed via `getComputedStyle` to be `@tailwindcss/forms`' own global default (12px/8px). Scoped down to 4px/6px via `.rental-review-aside .corex-input`, the same pattern already used by this codebase's own `.dr2-distribute .corex-input`/`.dr2-pipeline .corex-input` overrides — never a global forms-plugin change.
   - Amount column's real floor re-verified at the tighter padding against the same 9-character ceiling (R999,999.99): 100px → 90px, confirmed complete via screenshot, not assumed.
   - Date column re-verified the same way — Round 2's "118px still clipped the last digit of the year" finding was true at the OLD 12px padding; at 6px padding the full native date renders complete at 118px (was 130px), confirmed via screenshot.
   - Description cut ~20% per Johan's explicit number: 170px → 136px, verified against application 76's own real values ("salary"/"wages"/"utils" all still read as full words) — the existing `:title` tooltip is the documented backstop for values that don't, unchanged from its original role.
   - **Native `<input type="date">`'s display format is not page-controllable** — Johan asked for yy/mm/dd; the actual rendered format (confirmed live, not assumed: "06/24/2026" in this test environment) is the browser/OS locale's own rendering, and forcing a specific format would mean replacing the native control entirely, losing the native calendar picker, native validation, and the mobile date-wheel keyboard for two characters of width. Reported per Johan's own explicit ask ("tell me what the real options are before you rip it out"), not attempted — contributes 0 to this round's savings.
   - Total: 34 (desc) + 12 (date) + 10 (amount) = 56px off the row's real requirement. Default recomputed the same way as every prior round: 136 + 118 + 90 + 12 (gaps) + 24 (card padding) + 17 (scrollbar-gutter) = 397, rounded to 400. Floor/ceiling shifted the same -60px the default moved: 400/660 → 340/600. The freed 60px goes straight to `.rental-review-main` (already `flex: 1 1 auto`, needed no change of its own to absorb it) — the entire point, per Johan: "do not save 60px inside the panel and leave the panel 440px wide."

**Verified on application 76, default layout, no dragging, both roles** — the standard this feature has held to every round:
- Left panel: 190px → 90px. PDF page image: 474px → 634px (+34%) at the identical default settings, confirmed via `img.clientWidth` before/after (bisected with `git stash` against the same running server, not two different environments).
- Header: one visual row (17.5px height, unchanged from before — confirmed via screenshot, not just computed height) on both roles, "No property linked" appearing exactly once.
- Affordability rows: "salary"/"wages"/"utils" full words and R28,861.34/R29,340.99 fully rendered on the agent's editable grid AND the authoriser's separate read-only grid (its own grid, sized independently in an earlier round — re-checked here, not assumed to inherit the fix).
- **Mark re-projection held through the PDF width change** — a real pre-existing mark on document 2924 (not one drawn for this verification) was checked against an independent raster ground-truth crop (`page.width`/`page.height`-derived, not the DOM) both before and after: same text ("THE MANDATE COMPANY" + the letterhead swoosh) under the mark at both 474px and 634px display widths.

**One real mid-task bug, found and fixed before it ever reached a review**: a documentation comment referenced this file's own `@php` setup block by writing the literal text `` `@php` `` inside a Blade `{{-- --}}` comment. Blade treats `@php` as a raw/verbatim directive even inside comments — unlike `@if(...)` written as prose (which this file already had pre-existing, harmlessly, elsewhere) — and it silently swallowed roughly 400 lines of real template content between that comment and the next unrelated `@endphp`-shaped token, producing a `ParseError` at a completely unrelated line in the compiled output. Diagnosed by bisecting with `git stash` against a known-good baseline and tracking directive nesting depth through the actual compiled PHP, not by guessing from the error's own (misleading) line number. Fixed by never writing a bare Blade keyword as literal prose, even in a comment, even backtick-quoted.
...
- `app/Listeners/Contact/AddTenantTypeOnRentalApproval.php` — new listener, registered explicitly in `AppServiceProvider` (discovery is off agency-wide, AT-261)
- `app/Listeners/Contact/PromoteOwnerToSellerOnPropertyLink.php` — corrected its own stale "wired by discovery" docblock while here
...
## Round 6 — bottom-strip redesign, full-width document (2026-09-11, cc3)

Johan, after cc1 verified Round 5's tighter side-by-side layout: "not going to work. too little width left to properly see the pdf. Bank statements especially are printed small." Round 5 proved width alone could never solve this — the panel had to leave the side column entirely. Johan approved a mockup and said "Build it. His words: 'like it a lot, and if you then make the bar movable the agent can move it up and down to see more or less. their choice. Get it done. there is no ways the right panel is allowing this to work but the bottom panel can make it work.'"

**Structural change — the affordability panel is a bottom strip now, not a side column.** `.rental-review-columns` is always a vertical stack (`.rental-review-main` full width on top, `.rental-review-aside` full width below), at every viewport size — the old `@media (min-width: 1280px)` side-by-side breakpoint is gone entirely. `.rental-review-main`'s available height is `--rr-panel-h` (the existing measured-available-space calc) minus `--rr-strip-h` (the new strip height). The PDF viewer inside it now renders at roughly 1000-1066px wide at a 1522px viewport (was ~630-670px in every prior round) — the actual legibility win this whole redesign existed to deliver.

**Draggable HEIGHT, not width.** `rentalReviewLayout()`'s `startAsideResize()`/`asideWidth` (horizontal, `RA_ASIDE_*`) became `startStripResize()`/`stripHeight` (vertical, `RA_STRIP_*`: floor 260px — still shows the strip's header row + the ledger's own input row; default 320px — matches the approved mockup, four zones with ~3-4 ledger rows visible; ceiling 560px — document keeps genuinely readable height). Persisted via a separate `localStorage` key (`rentalReviewStripHeight`) — a browser's old width-drag value has no bearing on this new axis. Mark re-projection was proven to hold DURING the drag, not just before/after: a height-only drag never changes `.rental-review-main`'s width (it scrolls, it doesn't reflow), so the PDF's rendered size is unaffected by this drag by construction; the pre-existing per-page `ResizeObserver` mechanism (Round 4's own Item 7 fix) was separately re-verified to still re-project correctly through a genuine WIDTH change (a real viewport resize).

**Four zones, side by side inside the strip** (`.rr-strip-zones`, each zone independently role-gated rather than one big if/else wrapping the whole strip): **Period** (~150px — statement dates stacked vertically now that height is cheap and width isn't; entry count; unpaid flag) — **Entries** (flexible, the important part) — **Totals** (~196px — income/expenses/monthly figures, plus a "Max rent · N%" line that was Johan's own addition to the mockup, kept as one cleanly removable block) — **Actions** (~128px — buttons only; every field that used to sit permanently visible in the old column now opens in a modal, since 128px has no room for inline forms).

**The ledger merge — one table, not two cards.** Johan: "the agent must see income and expenses at the same time, the way they read on a bank statement." `incomeItems`/`expenseItems` stay two separate arrays server-side — same tables, same save()/audit/CRUD routes, unchanged — this is a display-layer merge only: a new `ledgerRows()` computed method merges both (filtered, sorted newest-first) for one **Date | Description | Income | Expense** table, each row wrapping a REFERENCE to the real item so in-place edits still mutate the real array and still save correctly. A row is income or expense, never both: the top input row (`newEntry`, decoupled from the old "trailing blank array row is the input" mechanic) disables the OPPOSITE amount field the instant one has a value — chosen over silently clearing what the agent already typed, per Johan's own instruction to make the interaction "obvious" rather than a validation error after the fact.

**One "Send back to applicant," not two.** Johan: "I also dont see the need for 2 section - send back for more info and reopen and send back. does the same function - send it back and ask for what you need." Investigation found the two ORIGINAL actions were genuinely different — `reopen()` changes status, unlocks the applicant's own typed fields, requires re-signature; `requestMoreInfoFromApplicant()` is notify-only, no unlock — and merging them without losing either capability meant deciding WHICH one fires per click rather than picking one to delete. The merged button/modal uses `canReopenNow` (computed server-side, the exact same `REOPENABLE_STATUSES` + override-tier gate the old Reopen button used) to always prefer the fuller action (reopen) when it's legally available, falling back to notify-only otherwise — the applicant never gets less than the old two-button screen could do. The modal's document checklist is derived from `RentalApplicationDocumentRequirement::checklistFor($agencyId, $employmentType)` — the SAME agency-configurable, per-employment-type mechanism the original application form's own printed checklist already runs on (never Johan's own mockup list verbatim, so it stays correct as an agency customises its requirements) — checked items get appended to the note as "Please also send: X, Y" before sending. The authoriser's OWN "Send back" (their pre-existing request-more-info-to-the-AGENT flow, never the applicant) is untouched — a genuinely separate flow that was never part of this merge.

**A new Alpine gotcha found and fixed.** Two adjacent `<template x-if="canReopenNow">bare text</template>` / `<template x-if="!canReopenNow">bare text</template>` elements — text content with no wrapping element — crashed Alpine's internal clone/scope-attach step the instant both existed in the DOM (`Cannot set properties of null (setting '_x_dataStack')`), reproducibly, regardless of the variable's value: Alpine's `x-if` clone mechanism needs the template's content to resolve to a single root ELEMENT (`content.firstElementChild`), not a bare text node. Diagnosed by additive bisection (starting from a known-good reverted state and adding pieces back one at a time, rather than subtracting from the broken state — subtractive single-piece removal kept "fixing" nothing until BOTH templates were removed together, which pointed at an interaction between them rather than either alone) after ruling out the CSS/JS shell restructure, the header `@php` block, and every individual ledger/send-back JS addition in isolation. Fixed by wrapping both in `<span>`, matching every other `x-if` on this page.

**Verified on application 76, default layout, no dragging, both roles:**
- PDF width: 634px (Round 5's own number) → 1066px at the identical 1522px viewport — confirmed via `img.clientWidth`, not the container.
- A real pre-existing mark (the same one Round 4/5 verified) re-checked against an independent raster ground-truth crop at the new width — still lands on "THE MANDATE COMPANY" — and again after a genuine viewport-width change (1522px → 1100px, forcing a real reflow), confirming the `ResizeObserver` re-projection mechanism survived the restructure.
- Ledger date column raised 100px → 118px after a live check showed a native `<input type="date">` clipping the year at 100px on real captured entries (application 76's own dates) — the same 118px figure Round 2/4/5 already established as the safe minimum; 100px was a fresh mistake in this round's first pass, not a regression of the old fix.
- Mutual exclusion confirmed live: typing into Income disables Expense (and vice versa) via the real DOM `disabled` attribute, not just visually.
- Strip height dragged from 320px through 500px in six real mouse-move steps, persisted across a page reload (`localStorage`).
- "Send back to applicant" modal opened, checklist showed the correct four documents for `employment_type = permanently_employed` (Payslip, Bank Statement, IDs / Identity, Proof of Residence), submitted successfully, correctly took the `reopen()` path (this application's status was `under_assessment`, in `REOPENABLE_STATUSES`) — confirmed via the resulting audit-trail entry, then the test application's status was reverted to its pre-verification value (`under_assessment`) since application 76 is a shared fixture other lanes rely on.
- Authoriser view: same four zones, same ledger merge (with its own status-dot/Auth-badge/strike-restore column), strike/restore toggle round-tripped live, Approve modal opened with the existing amount/reason fields intact.
...
Two tasks landed together because they're the same screen and the same goal, per Johan's own instruction.

### Continuous multi-document mark-up view

Johan, on application 107 after the PDF splitter turned one 17-page upload into five separate documents: "split the document, now it gives me all the pages to view and mark up. Can we still load this as 1 view and mark. Fica I think has the same scenario where it loads all the docs at once, not one at a time." Splitting is for FILING; marking up an affordability bundle (bank statement, then payslip, then ID) is ONE continuous job — the split had taken that away, forcing five separate open/mark/close cycles.

**Investigated FICA first**, per Johan's explicit instruction, before building anything. `compliance/fica/show.blade.php` renders EVERY uploaded document inline, stacked, on one page — an `<img>` for an image upload, a plain `<iframe>` for a PDF — never a click-through. That confirms the STRUCTURAL lesson (all documents together, not one-at-a-time behind a click) but NOT the rendering mechanism: an iframe delegates entirely to the browser's own native PDF viewer, which can't have a highlight/note overlay drawn on top of it, and loads the whole PDF up front with no progressive per-page loading at all — exactly what Johan separately warned against ("a real bundle will be worse"). The mechanism actually used here stays the existing per-page rasterised-image + SVG-overlay approach; only the STRUCTURE (all documents at once) is copied from FICA.

**Architecture — reuse, not fork.** Every inline-viewable document gets its own, fully independent `rentalDocumentHighlighter()` instance (the exact same factory the single-document click-to-view already used — see the shared script's own docblock: "hard-won, easy-to-reintroduce bugs... a second independently-maintained copy would risk drifting away from"). Because each instance owns its own `pages`/`marks`/`renderedPageSize`, a mark drawn in one document cannot leak into another's coordinate space — the 0–1 fraction system stays exactly as it was, per document, per page. Each instance is mounted via `x-init` (setting `activeDocId`/`firstPageUrl`/`remainingPagesUrl`/`postUrl`/`label` directly, then calling the factory's own existing `loadDocument()`) rather than adding a second entry point to the shared factory — zero changes to that file.

**Loading stays progressive at the document level, not just the page level.** Each document section only starts fetching once it scrolls within ~1000px of the CONTINUOUS VIEW's own scroll container (an `IntersectionObserver` scoped to that container, not the browser viewport, since this view is its own full-screen overlay) — then loads page 1 immediately and the rest behind it exactly like the single-document viewer already did, including the same "page 1 shown, N more loading" banner, per section. A 5-document/17-page bundle (application 107) mostly loads at once because all 5 sections sit within that margin on open; a much larger bundle would only load the first few sections until the agent actually scrolls further. Live-verified: the first two of the five documents (4 and 5 pages respectively) took longer to become interactive than the other three (1 page each) — normal first-time PDF-to-image conversion cost for documents never opened before, not a defect; each section shows its OWN independent "Loading document…" state while others are already interactive.

**The old single-document toggle is retired, not kept alongside.** The shared root component's `activeDocId`/sticky-header toolbar-swap (Highlight/Save/Done) is removed — Johan's own words framed the one-at-a-time click-through itself as the defect, so a second surviving way to do the same job would reintroduce it. The shared `rentalDocumentHighlighter()` factory function is completely unchanged; only the ROOT's own now-unused spread of it (activeDocId/pages/marks, never called from anywhere any more) went dormant — `markedUpDocIds` from that same spread stays live, since the per-row "Marked up" badge still reads it directly.

A jump-to sidebar (document names, click to scroll) sits alongside the continuous scroll area inside the same full-screen overlay, addressing Johan's explicit ask for a way to reach a specific document in a longer bundle.

**Verified on application 107, real 5-document bundle, both roles:**
- All 5 documents render in one continuous scroll, each with its own toolbar/legend/pages, following FICA's "all together" structure.
- A mark drawn on the first document (Rental Application.pdf, page 1) stayed at "1 mark" after scrolling all the way through the other four and back — no cross-document leakage.
- Saved, then verified on a completely fresh page load (new browser context, not just the same tab) — the mark was still there, in the same position, confirmed against the actual rendered image.
- Same bundle, same behaviour, on the authoriser's screen.

### Chrome-trim pass — Supporting Documents and the bottom strip

Johan's own words, the standing rule going forward: "on a working screen, every line of vertical space must be either DATA THE AGENT NEEDS or A CONTROL THEY ACT ON." He measured his own screen at 1522px and found the Supporting Documents block using ~338px for five filenames, and the bottom strip growing by "like 3 lines" when the ledger was empty.

**First pass measured, then found it barely moved the needle** (338px → 342.5px) — a real lesson worth keeping: trimming the FACTS inside an already-single-line row doesn't shrink the row's HEIGHT, because a readable row has a floor height regardless of how much text sits in it. Hoisting the repeated facts to one shared summary line ABOVE the list (the first attempt) also didn't help, because that summary line itself cost as much as the redundant text it replaced.

**Second pass, the one that actually landed:**
- The repeated "added by X · exact date · Added after submission" facts — identical on all five of application 107's rows, since they all came from the same split operation — are DROPPED outright when uniform across the set, not hoisted to a replacement line. Johan's own reasoning: "the audit trail already holds the full history, so this list does not need to be a second one." `$allSameOrigin` (computed fresh each render — a real check, not assumed, since a mixed batch of applicant + later agent uploads is common) now gates ONLY whether a row shows its own uploader/date/badge at all; when uniform, none do.
- The single "View & Mark Up all →" action moved INLINE into the always-visible heading row instead of its own separate line.
- The whole Supporting Documents block now defaults COLLAPSED (`docsOpen: false`) — Johan: "the agent needs it while marking up and rarely afterwards." The primary action stays reachable even collapsed (it's in the heading row, outside the `x-show`).
- Each row is genuinely one line now: filename, document type, a "Marked up" badge only where true, View & Mark Up, Download.
- Bottom strip: "Pick both dates to work out the number of months" (pure instruction, dead weight before AND after being read once) removed outright rather than folded in — the From/To field labels and the months count (which already appears once both are picked) covered everything it said. "0 entries captured" now only renders once there's something to count. The authoriser's read-only From/To lines folded from two lines each (a label, then the value) to one.

**Measured, same server, same viewport (1522px), `git stash` bisection for a true before/after:**
- Supporting Documents block: 338px → 58px collapsed (a -280px reclaim, handed straight to whatever sits above the fold — for most of a working session, the document view). Expanded (agent actively reviewing documents), still tighter per row than before: five genuinely single-line rows plus the heading, vs. the same five rows PLUS a hoisted summary line the first attempt had added.
- Bottom strip: the two named instructional/empty-state lines are conditionally hidden now (`x-show`), saving real height whenever the ledger is empty or the dates haven't been picked yet — the exact "3 lines for a fresh application" Johan named. The strip's own draggable height constants (`RA_STRIP_MIN/DEFAULT/MAX_PX`) were left unchanged: it's a user-persisted, manually-dragged value, not auto-sized to content, and changing the DEFAULT without live evidence it's now oversized for typical (non-empty) content would have been a guess, not a measurement.
- "Empty space to the right of the stacked date inputs" (Johan's own observation) did not reproduce in this measurement — both date inputs render at the full 150px zone width in this environment. Native `<input type="date">` rendering is OS/browser-dependent (Round 5 already hit this for the display FORMAT); flagged here rather than "fixed" against a symptom that couldn't be measured.
- "Dead area under the ledger's first row" is inherent to the approved mockup's own "reserve room for ~3 rows by default" decision (so the strip doesn't visibly resize every time an entry is typed) — not a new regression from this pass, and not changed here without Johan confirming he wants that reserve removed.

**What did not change, verified explicitly**: both agent and authoriser variants; marks land on the right words at every width (re-verified against the continuous view's own re-projection, not assumed from Round 6); the resizable divider and its `localStorage` persistence; five-digit amounts still render complete; full-word descriptions; autosave. The ledger's own behaviour (mutual exclusion, merge, totals) was not touched.
...
## Round 8 — capture-ledger rework: the highlighter mark IS the ledger line (2026-09-11, cc3)

Johan rejected Round 6's bottom strip outright — verbatim: "it stays split and disconnected." The ledger and the document it comes from were still two separate surfaces the agent had to keep in sync by hand. This round removes the transcription gap instead of moving it: an income/expense figure is captured by drawing on the document itself, at the exact figure — the mark IS the ledger row, not a summary of one typed elsewhere. Built in three stages so something real landed on QA1 early rather than one big-bang drop; Stage 1 is what's in this entry.

**Data — the existing highlighter mark record, extended, not replaced.** `rental_application_document_marks` (model: `App\Models\RentalApplicationDocumentMark`, persisted via `RentalApplicationDocumentHighlightService`/the new trait methods below) already carried the 0–1 fraction coordinate system, per-page resize re-projection, and mark-ownership governance (AT-401: "no in-place edit of a drawn mark, only draw-new and remove," for the mark's GEOMETRY). Migration `2026_09_11_100000_add_entry_fields_to_rental_application_document_marks.php` adds `entry_type` (enum income/expense/annotation, default annotation), `entry_date`, `entry_description`, `entry_amount`, and a new `rental_application_id` column so an entry can be queried without a document join; `document_id`/`page`/`type` are relaxed to nullable so an entry can exist **unanchored** (no drawn mark at all — reserved for the Stage 3 manual-entry path, not built yet). `source`/`confidence` on this table are pre-existing columns reserved for a future OCR decision Johan has not made — left untouched and unreferenced by this rework, per his explicit instruction.

**Old ledger data migrated, not dropped.** A second migration (`2026_09_11_100100_backfill_income_expense_items_to_unanchored_marks.php`) copies every active row out of `rental_application_income_items`/`rental_application_expense_items` into unanchored marks (no geometry, `rental_application_id` set directly) — **68 rows migrated: 45 income + 23 expense** (application 76 specifically: 6 income + 2 expense). A struck-out old row maps to a soft-deleted new mark, preserving the old "excluded from the total, kept for audit" behaviour as closely as the new read-only panel's simpler shape allows. The original two tables are untouched (copy, not move) — nothing about this migration is destructive, and no hard delete is used anywhere in it.

**Persistence — new endpoints, not the bulk-save path.** The existing `applyHighlights()`/`persistMarks()` mechanism replaces a document's WHOLE mark set on every "Save" click; a capture entry needs immediate, single-row persistence the instant its chip is confirmed, so three new endpoints on the shared `HandlesRentalApplicationDocumentMarks` trait (`captureEntryCreate`, per document; `captureEntryUpdate`/`captureEntryDelete`, per rental application — an entry outlives whichever document it was drawn on) write directly to the model, bypassing `applyMarks()` entirely:
- Agent: `POST .../{rentalApplication}/documents/{document}/capture-entries`, `PUT`/`DELETE .../{rentalApplication}/capture-entries/{markUid}`
- Authoriser: same paths under `.../authorisation/...`

`captureEntryUpdate` only ever touches `entry_date`/`entry_description`/`entry_amount` — never `points`/`width`/`highlighter_id` — so AT-401's "a mark's geometry never gets an in-place edit" rule stays intact for what the mark visually IS, while the CAPTURED FIGURE can be corrected like an ordinary ledger line. `captureEntryDelete` is a soft delete (non-negotiable #1) — the mark and its ledger line disappear together, since Round 8 makes them the same row. Ownership is the same rule `persistMarks()` already enforces for a plain mark: an unattributed (legacy/migrated) entry has nothing to protect; otherwise only its own author may change it, and an authoriser may never touch an agent's entry.

**Known accepted side effect of bypassing `applyHighlights()`:** the "Marked up" document badge (reads `highlighted_file_path`, only set by the burn step inside `applyMarks()`) won't light up for a document whose ONLY marks are capture entries, until a plain highlight/note is ALSO drawn and bulk-saved on it — which then correctly includes the capture marks in the burn too, since `persistMarks()` echoes existing rows rather than dropping them. Flagged, not fixed — fixing it would mean either running the burn step from the new endpoints too (defeating the "immediate, single-row" reason they exist) or decoupling the badge from the burn artifact entirely, neither of which this task asked for.

**Panel — read-only, fixed 196px, replaces the whole bottom strip.** `.rental-review-aside` reverts to a fixed side column (no drag, no CSS-var width, no resizer element) — Johan: "a tally, not a form, is what lets it stay narrow." Top to bottom: the pre-existing status banners (untouched); a statement-period section (two date fields, agent-editable/authoriser-read-only, `y/m/d`, per document SET not per line — unchanged from before, since a bank statement's own date range was never a per-row fact); grouped Income/Expense lists, one grid row each — a positional (never stored) numbered badge in a fixed purple/amber scheme, short date, right-aligned tabular-nums amount, and an inert jump-glyph (Stage 2 wires it; rendered now so the row shape doesn't change shape again when that lands); a totals block (Income, Expenses, Months, Monthly income, Net monthly — emphasised) that deliberately drops the old property-rent/30%-threshold verdict box, which lived in the deleted strip and was never part of this task's stated panel contents; and actions ("Add line manually," rendered but disabled with an explanatory tooltip — Stage 3, not built yet — plus the pre-existing send-back/submit/decide buttons, untouched).

**Rail — capture pens.** `RentalApplicationHighlighter` carries no persisted category column (`legacy_category` is a one-time migration-seed convenience, never a real field — see that model's own docblock). Income/Expense pens are identified by matching a highlighter's **label** ("Income"/"Expense", case-insensitive) — a pragmatic, flagged-as-fragile choice: an agency renaming its default Income/Expense highlighters would stop them acting as capture pens. Adding a real category column was ruled out of this task's scope. The rail now shows a "Capture" group (Income/Expense pens) separately from any OTHER highlighter an agency has configured (e.g. the default "Unpaid" pen, which stays a plain highlight, never a ledger entry) under its own "Highlight" heading — omitted entirely when there are none — and the Note pen is unchanged, under its own heading, as it already was.

**Draw → chip → save.** With a capture pen active, finishing a drag opens a ~330px chip (Date/Description/Amount, Amount focused+selected) anchored beside the pointer's own screen position — never over the mark, flipped above when there's no room below — instead of committing the stroke straight to the document's mark set. The stroke itself stays visible (rendered live from the still-unsaved pending mark) so the agent can see what they drew while typing; Enter (from any field) saves, Esc cancels **and drops the mark** — a capture-pen stroke is never added to the document's marks or its unsaved-changes state until the chip is actually confirmed, so there is nothing to undo on cancel. Saving calls the new create endpoint, then renders the mark (with its line-number badge) and updates the panel/totals live. Clicking an existing capture mark (its polyline already carried `data-mark-id` for the pre-existing hover-remove-× affordance) reopens the same chip in edit mode, with Update and Delete; a plain highlight or an annotation-typed mark keeps its old click-does-nothing/hover-× behaviour, unchanged.

**Wiring across two genuinely separate Alpine scopes.** `rentalCaptureLedger()` (the panel's tally logic — `captureEntries`, group filters, totals, `addCaptureEntry()`/`updateCaptureEntry()`/`removeCaptureEntry()`) is spread into the ROOT component (`rentalReview()`/`rentalAuthorisationViewer()`); the chip lives inside `rentalDocumentHighlighter()`, which — since Round 7's continuous view — is a genuinely separate `x-data` instance PER DOCUMENT, not merged with the root. The chip's confirm/delete handlers reach the panel via `$dispatch()`'d window events (`capture-entry-created`/`-updated`/`-deleted`), caught by `.window` listeners on the root element — the same bridge works regardless of how deeply the continuous view nests its per-document components.

**Silent-data-loss risk found and fixed before it could ship:** `saveAssessment()`'s pre-existing `syncItems()` treats an empty/absent `income_items`/`expense_items` array as "the list is now empty, delete everything not present." Removing the old ledger's client-side state without also removing this handling would have soft-deleted every existing income/expense-item row on the very next autosave of anything else on the screen (statement dates, notes). Fixed by removing the endpoint's income/expense handling entirely — capture now happens via marks, never through this endpoint. `syncItems()` itself is left in place, dead, with a comment explaining why (it operates on the now-bypassed item tables, which are out of scope for this task to remove).

**Explicitly out of scope, per Johan's own instruction:** OCR — not built, not stubbed, no field added for it. The agent types the amount.

**Explicitly deferred to Stage 2/3 (not built in this pass):** clicking a panel row to jump to and flash its mark on the document (the panel's jump-glyph is rendered but inert); "Add line manually" for figures with nothing to highlight (rendered but disabled, with a tooltip explaining why).

### Files changed

- `database/migrations/2026_09_11_100000_add_entry_fields_to_rental_application_document_marks.php` — new columns + relaxed nullability, described above.
- `database/migrations/2026_09_11_100100_backfill_income_expense_items_to_unanchored_marks.php` — one-time copy, 68 rows.
- `app/Models/RentalApplicationDocumentMark.php` — `entry_type`/`entry_date`/`entry_description`/`entry_amount`/`rental_application_id` added to fillable/casts; `rentalApplication()` relation; `isLedgerEntry()`/`isAnchored()` helpers; `toMarkArray()` extended.
- `app/Http/Controllers/Concerns/HandlesRentalApplicationDocumentMarks.php` — `captureEntryCreate`/`captureEntryUpdate`/`captureEntryDelete`, `guardCaptureEntryOwnership()`.
- `app/Http/Controllers/CoreX/RentalApplicationReviewController.php` — `$captureEntries` query in `show()`; `saveAssessment()`'s income/expense handling removed.
- `app/Http/Controllers/CoreX/RentalApplicationAuthorisationController.php` — `$captureEntries` query in `show()`; the now-dead `$serializedIncomeItems`/`$serializedExpenseItems` (only ever consumed by the removed `rentalAssessmentEditor()` wrapper) removed rather than left unused.
- `routes/web.php` — 6 new named routes, agent + authoriser, for capture-entries create/update/destroy.
...
### Stage 2 — panel row click-to-jump (2026-09-11, cc3)

Clicking a panel row (the whole row, per Johan's own spec, not just the jump glyph) opens the document the line came from, scrolls its mark into view, and flashes it. If the document is already open in the continuous view, it just scrolls and flashes — `openContinuousView()` toggling an already-open overlay and re-scrolling to a document already in view is a no-op/harmless re-scroll, not a special case that needed its own branch.

An unanchored row (a migrated old-ledger entry, or a future Stage-3 manual entry) has no `document_id` — its row renders with an inert glyph and does nothing on click, per the panel's own stated design ("shows without a mark number and with no jump target"); `page` was added to `toMarkArray()`'s wire contract (previously omitted — every OTHER consumer already gets a mark's page from the outer `{pageIndex: [...]}` key of the bulk-load response, so this is purely additive) so an anchored row carries enough to find its mark.

**Two bridges across three genuinely separate Alpine scopes**, since the row (`rentalCaptureLedger()`, spread into the root), the continuous-view toggle (`rentalReviewLayout()`, one level down from the root), and the actual per-document mark data (`rentalDocumentHighlighter()`, a THIRD, independent instance per document, another level down) don't share `this`:
- `jumpToMark(row)` lives on `rentalReviewLayout()` (not `rentalCaptureLedger()`) specifically because it needs `openContinuousView()`, which only exists there — the row's `@click="jumpToMark(row)"` resolves it via Alpine's normal ancestor-scope walk (a template expression, not a plain method calling `this.foo()`, so the walk works regardless of which of the three scopes actually owns each name).
- Reaching the ONE per-document `rentalDocumentHighlighter()` instance that actually owns the mark needs a real bridge, since there can be several (one per document) and none is a scope ancestor of the row: `jumpToMark()` dispatches a `rental-jump-to-mark` window event (`{documentId, markId}`); every `rentalDocumentHighlighter()` instance listens for it (registered once, inside `initHighlighterPrefs()` — already called by every instantiation site) and no-ops unless `documentId` matches its own `activeDocId`.

**Flash, without fighting the SVG's own re-render.** `strokesSvgFor()` regenerates its whole `<svg>` from a markup STRING on every reactive dependency change (see that function's own docblock) — an imperative `classList.add()`/style mutation on the polyline would get silently wiped the next time ANYTHING else it depends on changes (a resize, a hover, an unrelated mark added). Made a proper reactive dependency instead: a `flashMarkId` property, read inside `strokesSvgFor()` itself (so Alpine tracks it and re-runs the function when it changes, exactly like `renderedPageSize` already does for a resize), adding a `rah-mark-flash` CSS class to the matching polyline for ~1.3s. Reduced motion (Johan's spec verbatim: "respect `prefers-reduced-motion` — outline instead of animation") is a `@media (prefers-reduced-motion: reduce)` override on that same class swapping the pulsing `@keyframes` animation for a static, high-contrast outline stroke instead — no JS branching on the media query at all.

**Progressive-load race, handled, not assumed away.** A document scrolled into view for the first time hasn't loaded yet — `openContinuousView()`'s own `scrollIntoView()` is what triggers its `IntersectionObserver` to start `loadDocument()`, but that happens async and isn't causally ordered with the jump. `waitUntilPagesReady()` polls `this.loading`/`this.pagesLoading` (bounded, so a document that genuinely fails to load doesn't hang the jump forever) before querying for the mark's DOM node — never a second concurrent `loadDocument()` call, which isn't safe to make twice.

**Files changed (Stage 2, in addition to the list above):**
- `app/Models/RentalApplicationDocumentMark.php` — `page` added to `toMarkArray()`.
- `resources/views/corex/rental-applications/review.blade.php` — panel rows gain `@click="jumpToMark(row)"` (guarded on `row.document_id`) and an active/inert jump-glyph colour; `jumpToMark()` added to `rentalReviewLayout()`.
- `resources/views/corex/rental-applications/partials/document-highlighter-script.blade.php` — `rental-jump-to-mark` window listener (registered in `initHighlighterPrefs()`), `waitUntilPagesReady()`, `flashMarkId`/`scrollToAndFlashMark()`; `strokesSvgFor()`'s `poly()` helper gains a flashing flag.
- `resources/views/corex/rental-applications/partials/document-highlighter-pages.blade.php` — `.rah-mark-flash` CSS (animated pulse, reduced-motion override).
