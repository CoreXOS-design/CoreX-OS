...
**Status:** ROUND-3 ON STAGING — awaiting Johan's final full eyeball before ship-to-live. Round-1 (items 1,3,4,5,6,7 + agency-settings UI + comp engine corrected over 3 Staging-feedback rounds + curation toolkit/AT-21 + holding-cost transparency) and Round-2 (5 PDF/data fixes) all landed.

**Round-3 — Market Overview re-framed to the MIC single-source design (verified on PRES 87, Uvongo populates: 2026, 7 sales, R1.3M median, R1.295M–R1.7M range):** The MIC-pull architecture (`MicSnapshotHydrator::hydrateSuburbMetrics` reading `market_data_points` by suburb) was already built/wired — but the MIC held price ranges and ZERO median/sales-count for any suburb because `CmaInfoMedianSalesAnalysisParser`'s Sales-Analysis triplet regex required a change-% column the "ST Residential Sales Analysis" table lacks. (1) Parser now falls back to the Residential Price Ranges row (year|count|low|median|high|max) for `suburb_median_price_year` + `suburb_sales_count_year` when the triplet didn't resolve a median. (2) `hydrateSuburbMetrics` robust — anchors on the latest year with ANY suburb datapoint, surfaces each metric independently, no median-required return-0 bail. (3) PDF empty-state reframed to "No market data for {suburb} yet — import via Market Intelligence" + link, shown only when the MIC genuinely has nothing. (4) Evidence UI primary path is now an "Import via Market Intelligence" link (reusable by suburb); the per-presentation upload is a labelled legacy fallback. **PRODUCTION DATA STEP after ship:** existing suburb reports (e.g. report:30 Uvongo) must be re-parsed on live via the MIC re-parse (which resets cleanly) so their median+count populate `market_data_points`. My round-2 "No suburb report uploaded" framing was wrong and is removed.
 **Round-2 (5 PDF/data items, all investigate-first + verified on PRES 87 v180):** (1) Market Overview → honest "No suburb report uploaded" empty-state when none attached (PRES 87 has 0 uploads; parser binds fine when one is); (2) Active Competition table + footer average unified onto `competitor_stock['visible']` (the scored prospecting_listings the cards/pins/count already use — table now shows the correct R2.35–3.2M set, was 2 stale R1.39/1.52M portal rows); (3) section numbering → sequential in reading order via a `__SECNO__` placeholder + one document-order sweep in `buildHtml` (replaced literals frozen to source order; beats now monotonic; `$sectionAfterInflow` deleted); (4) pagination → spatial legend repeats header + rows don't split, holding-cost grid protected with `page-break-inside:avoid`; (5) subject map → message now distinguishes "key not configured" vs "key set but Maps Static API not enabled" — **the actual enablement is Johan's Google Cloud action** (`GOOGLE_STATIC_MAPS_API_KEY` env on staging + enable Maps Static API on the key's project). Item 6 built ahead of its ticket — **AT-26 = "done early"**.
