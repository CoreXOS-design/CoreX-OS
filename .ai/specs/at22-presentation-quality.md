...
**Status:** ROUND-2 ON STAGING — awaiting Johan's eyeball. Round-1 (items 1,3,4,5,6,7 + agency-settings UI + comp engine corrected over 3 Staging-feedback rounds + curation toolkit/AT-21 + holding-cost transparency) all landed. **Round-2 (5 PDF/data items, all investigate-first + verified on PRES 87 v180):** (1) Market Overview → honest "No suburb report uploaded" empty-state when none attached (PRES 87 has 0 uploads; parser binds fine when one is); (2) Active Competition table + footer average unified onto `competitor_stock['visible']` (the scored prospecting_listings the cards/pins/count already use — table now shows the correct R2.35–3.2M set, was 2 stale R1.39/1.52M portal rows); (3) section numbering → sequential in reading order via a `__SECNO__` placeholder + one document-order sweep in `buildHtml` (replaced literals frozen to source order; beats now monotonic; `$sectionAfterInflow` deleted); (4) pagination → spatial legend repeats header + rows don't split, holding-cost grid protected with `page-break-inside:avoid`; (5) subject map → message now distinguishes "key not configured" vs "key set but Maps Static API not enabled" — **the actual enablement is Johan's Google Cloud action** (`GOOGLE_STATIC_MAPS_API_KEY` env on staging + enable Maps Static API on the key's project). Item 6 built ahead of its ticket — **AT-26 = "done early"**.
