...
**CORRECTION, 2026-09-16 — the build that followed §1.2 did NOT actually fix this, and I reported it
as fixed when it wasn't. Recorded here in full because Johan asked for facts, not the earlier
optimistic version.** The build added `lease_start`/`lease_end`/`street_address`/`erf_no` as extra
keys to check in `$document->fields_json`, based on reading `lease-agreement-popi-v8.blade.php` — a
static preview file reached only via `WebTemplateController`'s dev-preview routes
(`docuperfect.webPreview.leaseAgreementPopiV8`), which I later confirmed are not called from anywhere
in the real document-creation flow (`DocumentController::store()`/`edit()`). I built the fix against
the wrong artifact.

**What `fields_json` actually is on a real document**, checked directly against a live QA1 document
(`docuperfect_documents.id=405`, `template_id=48`, the current active residential template): a
**numerically-indexed array of field-descriptor objects**, each carrying `named_field_id`,
`named_field_name` (e.g. `"Lessor Name"`, `"Amount"`, `"Lease date"`), position/size/page data, and —
once an agent fills it in — a `value` key. It is never a flat map like `['lease_start' => '...']`.
Proven empirically, not just read: calling the landed `extractLeaseFields()` against a hand-built
document using this real shape returns `null`/`0` for every field, exactly as it would against any
real signed lease document today.

**The real field catalogue on QA1's two active lease templates** (queried directly from
`templates.fields_json`, not guessed):
- **Template id 48, "Lease agreement - (V8)"** (`document_types.slug = rental_agreement`,
  `template_type = rental`) — the active residential template (id 20, "2026 Lease agreement - (V8)",
  is the archived predecessor, `archived_at` 2026-03-06). 67 field instances, 13 distinct labels:
  `Lessor Name`, `Lessor Address`, `Lessor ID`, `Lessee Name`, `Lessee Address`, `Lessee ID`,
  `Property Number`, `Property Address`, `Complex`, `Number`, `Amount`, `Lease date`, `Expiry Date`.
- **Template id 23, "Commercial Lease agreement (V5)"** (`document_types.slug = rental_agreement`,
  `template_type = rental`) — 48 field instances, but only **6** carry a `named_field_name` at all:
  `Lessor`, `Lessor Address`, `Lessor ID`, `Lessee Name`, `Lessee Address`, `Lessee ID`. No labelled
  amount/date/property fields were found on this template as currently configured on QA1 — flagged as
  a real, unexplained gap, not glossed over: either those 42 unlabelled fields carry the financial/date
  data some other way this investigation didn't find, or the commercial template is missing labels its
  residential sibling has. Not resolved here.

**A third, independent mismatch found in the same investigation, also unfixed:** the e-sign wizard's
own manual-field prefill (`ESignWizardController.php:5919-5924`) hardcodes
`'Lease Start' => 'lease_start'` / `'Lease End' => 'lease_end'` as the labels it recognises — but
template 48's real labels are `Lease date` / `Expiry Date`, not `Lease Start` / `Lease End`. So even
the WIZARD'S OWN prefill mechanism, independent of anything cc5 built, doesn't recognise this
template's actual date-field labels.

**Confirmed NOT blocked**, per Johan's direct question: `Template::isEsignBlocked()`
(`app/Models/Docuperfect/Template.php:601-650`) blocks on a fixed slug list
(`otp`, `sale_agreement`, `deed_of_sale`, `deed_of_alienation`, `offer_to_purchase`) and a name-regex
tuned for sale/purchase/OTP/deed-of-sale/koopkontrak language — checked directly against all three
lease templates: `isEsignBlocked()` returns `false` for ids 48, 23, and 20. A lease agreement is not
caught by the Alienation of Land Act guard. This is the one part of today's investigation that is
unambiguously good news.

**Design constraint this imposes on any future Lease → document integration (NOT built now, per
explicit instruction — Johan flagged that live's actual template may differ from QA1's, and we have
no live access to verify it, ever):**
- Never bind to a hardcoded `template_id`. The lease template a future "generate agreement" action
  uses must be resolved per agency, per lease type (residential/commercial), through an
  agency-configurable setting — the same shape as every other agency-configurable choice in this
  spec — precisely because QA1's ids (48, 23) are QA1's ids; live's are unverifiable and may not
  match, and even QA1's own template's actual field labels turned out to differ from what an
  earlier pass assumed without checking.
- Never bind to a hardcoded field-name list. Mapping from a `Lease` record's own fields
  (property/tenant/rent/dates) to a template's `named_field_name` labels must be data-driven —
  resolved by reading the target template's actual field catalogue at the time of use (the same
  query used to produce the list above), not a fixed PHP array of expected labels. A fixed list
  works today and silently breaks the moment an agency's template — or live's — uses different
  wording, exactly as `Lease Start` vs `Lease date` already does today, in code nobody today
  proposed to touch.
- Whatever gets built must therefore include, as part of the same piece of work, actually fixing
  `extractLeaseFields()`/`firstPartyWithRole()` to read the real `named_field_name`/`value` shape —
  the currently-landed fix does not do this and cannot be treated as a foundation to build on
  without that correction first.
