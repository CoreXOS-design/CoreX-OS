...
**Carried over unchanged across both passes, NOT re-verified:** Contacts, Properties, Rental Applications, Leases, Core Matches, Buyer Pipeline, Presentations/CMA, DR2, Deeds Capture, E-sign/DocuPerfect, Portal Syndication, Communication Mailboxes, both Phase 2 working-surface writeups, and Phase 3 findings D3–D9 and A1–A7. These sections read as confident, cited prose — exactly like Rental Inspections and D1 did before each was found wrong. Their citations were spot-checked only where a correction elsewhere happened to touch the same code. **Do not treat "not yet flagged" as "verified."** Two headline claims in this document (Rental Inspections' non-existence, D1's "none shared") have now both been wrong on inspection — that is a pattern, not a coincidence, and it should weight how much confidence the *un*-rechecked sections are given.
...
- `RentalWorkOrderService::notifyCreated`/`notifyCompleted`/`notifyOverdue` — confirmed to exist, not traced to their recipients.
