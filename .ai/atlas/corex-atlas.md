...
**Corrected by:** cc3 (audit, 2026-09-25) found this document wrong about an entire shipped pillar (Rental Inspections) and thin on Agent/User, one unverified duplication finding (D2), and 18 never-touched Settings controllers. This revision was built directly against the `origin/QA1` checkout at commit `d1f4296bc` (2026-09-24) — the same commit cc3 was reporting on — to re-verify every corrected claim against real, current code rather than reasoning from the original pass's (wrong) premise that this checkout predated the work in question.
...
This section exists because the previous version of this file asserted a shipped feature didn't exist, and nobody caught it before it could mislead a design decision. Future work gets built on top of this document — it has to state its own reliability rather than look uniformly authoritative.

**Re-traced against real code in this pass (2026-09-25), every claim carrying `file:line`:**
...
**Carried over unchanged from the original pass, NOT re-verified in this correction:** Contacts, Properties, Rental Applications, Leases, Work Orders, Core Matches, Buyer Pipeline, Presentations/CMA, DR2, Deeds Capture, E-sign/DocuPerfect, Portal Syndication, Communication Mailboxes, both Phase 2 working-surface writeups, and Phase 3 findings D1, D3–D9 and A1–A7. These sections read as confident, cited prose — exactly like the Rental Inspections section did before it was found to be wrong. Their citations were spot-checked only where a correction elsewhere in this pass happened to touch the same code (e.g. the Contact-search work under D2 also touched `CoreX\ContactController`, which is unchanged and confirmed accurate). **Do not treat "not flagged by cc3" as "verified."** cc3 audited for one wrong pillar, a thin pillar, one weak finding, and one uninvestigated area — that is not the same as re-deriving all ~20 sections from code.

**One thing worth naming here even though it wasn't in cc3's brief:** while re-checking Rental Inspections, the sibling "Scope note on Rental Inventory" in the original header (claiming no inventory feature exists in any rental/property-condition sense) was also found false in the same pass, by the same mechanical check (`corex.rental-inventories.*`, `RentalInventoryController`, `RentalInventorySetting`, etc. all exist and are routed — see the Settings section's `RentalInventorySettingsController` entry and the note under Rental Inspections below). It has been corrected to state only what was confirmed — that the module exists — without expanding it to full Phase 1 depth, since that was not asked for in this pass and going beyond the brief risks the same shallow-but-confident failure mode this whole correction exists to fix. It is listed as unwalked below.

**Not walked at all in this pass — flagged, not guessed:**
- Rental Inventories (`corex.rental-inventories.*`, `RentalInventoryController`/`RentalInventoryRecordingController`/`RentalInventoryCaptureController`) — confirmed to exist via routes/models/settings controller only; no Phase 1 map entry.
- Rental Fault Reports (`corex.rental-fault-reports.*`, `RentalFaultReportController`) and Rental Work Orders (`corex.rental-work-orders.*`, `RentalWorkOrderController`) — confirmed to exist via routes only, both distinct from the DealV2-era "Work Orders" section already in this document; no Phase 1 map entry. **This means the existing "Work Orders" section of this document is now itself incomplete** — it describes only the DR2/DealV2 pipeline work-order panel and has no knowledge of this separate rental-specific work-order module. Not corrected in this pass (outside cc3's four findings) — reported to Johan separately rather than fixed here.
- Phase 3 finding D1 (property search) was not re-audited. While verifying D2, it became clear the four real contact-search implementations share one canonical scope (`Contact::scopeSearch()`, `app/Models/Contact.php:583`) rather than being independently reimplemented at the query-logic level — only the endpoint/UI layer is duplicated. D1 makes the stronger claim ("each its own controller method... none shared") for the equivalent property-search case; whether that holds under the same scrutiny was not checked here.
...
**Reads from other pillars**
- Property — every inspection is created against a `Property`; `chainTailFor(Property $property)` (`RentalInspection.php:543`); the property show page's Rental tab includes the live recording partial keyed to the property (`show.blade.php:5087,5090` — `@include('corex.properties.partials.rental-inspection-recording', ...)`).
...
**Where it actually lives:** hybrid, not "embedded only" as the original section claimed — a standalone module with its own sidebar entry and searchable index (`corex.rental-inspections.index`, sidebar `corex-sidebar.blade.php:1099`) for the agency-level record, PDFs, and public link, **and** a live recording surface embedded inline inside the Property show page's single "Rental" tab (`show.blade.php:1415`, key `'rental'`) alongside the still-present older rental-images gallery (`partials/rental-section-body.blade.php`, `show.blade.php:5520`).

**Confirmed NOT walked in this pass:** the sibling **Rental Inventories** module (`corex.rental-inventories.*`, routes/web.php:3291-3327 — `RentalInventoryController`/`RentalInventoryRecordingController`/`RentalInventoryCaptureController`, models `RentalInventory*`) exists and is routed — the original header's blanket claim that no "inventory" feature exists in this sense was also false — but it was not investigated to Phase 1 depth here; see Confidence and gaps at the top of this file. Per its own route-file comment (routes/web.php:3290-3292), it is deliberately "its own document (never a tab on RentalInspection)."
