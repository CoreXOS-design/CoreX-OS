...
class NotificationEventTypeSeeder extends Seeder
{
    ...
    private function catalogue(): array
    {
        return [
            ...
            $this->row('property.no_activity', 'property', 'Activity', 'No activity since listing', 'days', 21, 3, 180, 2),
            $this->row('property.compliance_doc_missing', 'property', 'Compliance', 'Compliance documents missing', 'hours', 48, 1, 720, 3),
            // ── DELIBERATELY ABSENT — RETIRED (AT-235 R1) ────────────────────────
            //   contact.fica_missing    retired 19 Jun (the 1.9M storm — killed by hand)
            //   contact.fica_expiring   retired here
            //   contact.no_followup     retired here
            //
            // Their only producer (ScanContactNotifications) was deleted on 1 Jul. They
            // are absent from this seeder so a FRESH environment never creates them —
            // the accompanying migration retires them on environments that already have
            // them. The seeder is the statement of what SHOULD exist; the migration is
            // the correction for what already does.
            ...
            $this->row('deal.documents_missing', 'deal', 'Documents', 'Required deal documents missing', 'hours', 24, 1, 720, 11),
            $this->row('deal.commission_unpaid', 'deal', 'Finance', 'Commission overdue', 'days', 30, 1, 180, 12),
            $this->row('deal.milestone_due', 'deal', 'Lifecycle', 'Deal milestone due', 'hours', 24, 1, 168, 13),
            ...
            $this->row('leave.cancelled', 'agent', 'Leave', 'Leave application cancelled', 'none', null, null, null, 23),
