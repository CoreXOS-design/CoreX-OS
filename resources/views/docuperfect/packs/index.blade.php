            ...
            <div class="pack-card rounded-md p-4 flex flex-col"
                ...
                <div class="text-xs mb-3 flex flex-wrap items-center gap-2" style="color: var(--text-muted);">
                    @if($pack->usesSlots())
                        ...
                        <span>&middot;</span>
                        <span style="color: {{ $pack->creation_mode === 'linked' ? 'var(--brand-icon, #0ea5e9)' : 'var(--text-muted)' }};">{{ ucfirst($pack->creation_mode) }}</span>
                    @else
                    ...
                    <ul class="text-xs space-y-1" style="color: var(--text-secondary);">
                        ...
                        <li class="flex items-center gap-1.5">
                            @if($slot->slot_type === 'required')
                                <span class="w-1.5 h-1.5 rounded-full flex-shrink-0" style="background: var(--ds-green, #059669);"></span>
                            @elseif($slot->slot_type === 'selectable')
                                <span class="w-1.5 h-1.5 rounded-full flex-shrink-0" style="background: var(--ds-amber, #f59e0b);"></span>
                            @else
                                <span class="w-1.5 h-1.5 rounded-full flex-shrink-0" style="background: var(--brand-icon, #0ea5e9);"></span>
                            @endif
                            {{ $slot->label }}
                        </li>
                        @endforeach
                ...
                <div class="flex flex-wrap items-center gap-2 mt-auto pt-3" style="border-top: 1px solid var(--border);">
                    ...
                    <form method="POST" action="{{ route('docuperfect.packs.destroy', $pack->id) }}" class="inline ml-auto" onsubmit="return confirm('Delete this pack?');">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="text-xs font-semibold transition-colors" style="color: var(--ds-crimson, #c41e3a);">Delete</button>
                    </form>
                    @endif
                </div>
            </div>
...
<style>
    .pack-card:hover { border-color: var(--brand-icon, #0ea5e9); }
</style>
@endsection
