    ...
    >
        ...
        <div class="ds-status-card p-4 space-y-3">
            ...
            <div class="space-y-2">
                <label class="flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-colors"
                       :class="creationMode === 'linked' ? 'border-[color:var(--brand-icon)] bg-[color:var(--surface-2)]' : 'border-[color:var(--border)] hover:bg-[color:var(--surface-2)]'">
                    <input type="radio" name="creation_mode" value="linked" x-model="creationMode"
                           class="mt-0.5 rounded-full border-[color:var(--border)] text-[color:var(--brand-icon)]">
                    <div>
                        <div class="text-sm font-semibold" style="color:var(--text-primary)">Linked Pack</div>
                        <div class="text-xs" style="color:var(--text-muted)">Documents share named fields. Fill once, populate everywhere.</div>
                    </div>
                </label>
                <label class="flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-colors"
                       :class="creationMode === 'individual' ? 'border-[color:var(--brand-icon)] bg-[color:var(--surface-2)]' : 'border-[color:var(--border)] hover:bg-[color:var(--surface-2)]'">
                    <input type="radio" name="creation_mode" value="individual" x-model="creationMode"
                           class="mt-0.5 rounded-full border-[color:var(--border)] text-[color:var(--brand-icon)]">
                    <div>
                        <div class="text-sm font-semibold" style="color:var(--text-primary)">Individual Documents</div>
                        <div class="text-xs" style="color:var(--text-muted)">Each document is standalone. Named fields don't sync.</div>
                    </div>
                </label>
            </div>
        </div>
        ...
        <div class="ds-status-card p-4 space-y-3">
            ...
            <div class="flex items-center gap-2">
                ...
                <label for="packGlobal" class="text-sm" style="color:var(--text-secondary)">Global (all branches)</label>
            </div>
            <div>
        ...
        <div class="ds-status-card p-4 space-y-3">
            ...
            <div class="text-xs mb-2" style="color:var(--text-muted)">Define what the pack contains. Each slot can be a required template, a selectable type, or a Knowledge Base attachment.</div>

            <template x-if="slots.length === 0">
                <div class="text-center text-sm py-6 border border-dashed rounded-lg" style="color:var(--text-faint);border-color:var(--border)">
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex flex-col gap-0.5 pt-1">
                                <button type="button" @click="moveUp(idx)" :disabled="idx === 0"
                                        class="text-[color:var(--text-faint)] hover:text-[color:var(--text-secondary)] disabled:text-[color:var(--border)] p-0.5">
                            ...
                            <div class="flex-1 space-y-3">
                                ...
                                <div x-show="slot.slot_type === 'selectable'" class="space-y-2">
                                    ...
                                    <div class="text-xs" style="color:var(--text-faint)">Agent will see all templates of this type and choose which to include.</div>
                                    <div class="flex items-center gap-4">
                                        <label class="flex items-center gap-1 text-xs" style="color:var(--text-secondary)">
                                            <input type="checkbox" x-model="slot.allow_multiple" class="rounded text-sm" style="border-color:var(--border)">
                                            Allow multiple
                                        </label>
                                        <label class="flex items-center gap-1 text-xs" style="color:var(--text-secondary)">
                                            <input type="checkbox" x-model="slot.is_optional" class="rounded text-sm" style="border-color:var(--border)">
                                            Optional (can skip)
                                        </label>
                                    </div>
                                </div>
                                ...
                                <div x-show="slot.slot_type === 'attachment'" class="space-y-2">
                                    ...
                                    <div class="text-xs" style="color:var(--text-faint)">Agent will see all documents in this category and choose which to attach.</div>
                                    <div class="flex items-center gap-4">
                                        <label class="flex items-center gap-1 text-xs" style="color:var(--text-secondary)">
                                            <input type="checkbox" x-model="slot.allow_multiple" class="rounded text-sm" style="border-color:var(--border)">
                                            Allow multiple
                            ...
                            {{-- Delete slot --}}
                            <button type="button" @click="removeSlot(idx)" class="text-[color:var(--text-faint)] hover:text-red-500 p-1 mt-1" title="Remove slot">
            ...
            <button type="button" @click="addSlot()" class="inline-flex items-center gap-1 text-xs font-medium text-[color:var(--brand-icon)] hover:opacity-80 mt-2">
        ...
        <div class="ds-status-card p-4" x-show="slots.length > 0">
            ...
            <div class="text-xs mb-2" style="color:var(--text-muted)">When an agent launches this pack, they will:</div>
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('docuperfect.packs.index') }}" class="text-sm text-[color:var(--text-muted)] hover:text-[color:var(--text-secondary)]">Cancel</a>
        </div>
    </form>
