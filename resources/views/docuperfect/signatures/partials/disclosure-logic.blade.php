            ...
            checklists.forEach(checklist => {
                ...
                checklist.querySelectorAll('.corex-disclosure-row').forEach(row => {
                    ...
                    radios.forEach(radio => {
                        ...
                        } else if (self.disclosureMarksLocked && self._signerIsDisclosingParty()) {
                            // AT-303 Stage 2 — a DOWNSTREAM owner recipient may not
                            // silently overwrite the locked mark; tapping a radio
                            // selects a PROPOSED new value (dashed), then the
                            // "Propose change" button captures their initial and
                            // routes the amendment back to the earlier party.
                            radio.style.cursor = 'pointer';
                            radio.addEventListener('click', () => {
                                radios.forEach(r => {
                                    r.style.outline = '';
                                    r.removeAttribute('data-proposed');
                                });
                                radio.style.outline = '2px dashed #f59e0b';
                                radio.setAttribute('data-proposed', 'true');
                                row.setAttribute('data-proposed-value', radio.dataset.value || '');
                            });
                        } else {
                    ...
                    // AT-303 Stage 2 — attach the "Propose change" affordance for a
                    // downstream owner recipient (once per row).
                    if (self.disclosureMarksLocked && self._signerIsDisclosingParty()
                        && !row.querySelector('.corex-propose-btn')) {
                        const stmtEl = row.querySelector('.corex-disclosure-statement');
                        const statement = stmtEl ? stmtEl.textContent.trim() : rowKey;
                        const btn = document.createElement('button');
                        btn.type = 'button';
                        btn.className = 'corex-propose-btn';
                        btn.textContent = 'Propose change';
                        btn.style.cssText = 'margin-top:4px;font-size:11px;padding:2px 8px;border:1px solid #f59e0b;'
                            + 'border-radius:6px;background:#fffbeb;color:#92400e;cursor:pointer;white-space:nowrap;';
                        btn.addEventListener('click', () => {
                            const proposed = row.getAttribute('data-proposed-value') || '';
                            if (typeof self.proposeDisclosureChange === 'function') {
                                self.proposeDisclosureChange(rowKey, statement, proposed);
                            }
                        });
                        (stmtEl || row).appendChild(btn);
                    }
