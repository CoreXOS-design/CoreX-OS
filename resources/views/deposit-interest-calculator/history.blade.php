...
<div class="w-full space-y-5">
    ...
    <div class="rounded-md px-4 py-3" style="background: var(--surface); border: 1px solid var(--border);">
        <form method="GET" action="{{ route('deposit-interest-calculator.history') }}" class="flex flex-wrap items-center gap-3">
            ...
            @if(request('search'))
                <a href="{{ route('deposit-interest-calculator.history') }}" class="text-xs font-semibold" style="color: var(--brand-icon, #0ea5e9);">Clear</a>
            @endif

            <span class="ml-auto text-xs" style="color: var(--text-muted);">
                Showing {{ number_format($calculations->count()) }} of {{ number_format($calculations->total()) }}
            </span>
        </form>
    </div>
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="{{ route('deposit-interest-calculator.show', $calc) }}"
                                       ...
                                       onmouseover="this.style.color='var(--brand-icon, #0ea5e9)'" onmouseout="this.style.color='var(--text-muted)'">
                                    ...
                                    <form method="POST" action="{{ route('deposit-interest-calculator.destroy', $calc) }}"
                                        ...
                                        <button type="submit" class="p-1 rounded transition-colors" style="color: var(--text-muted);" title="Delete"
                                                onmouseover="this.style.color='var(--ds-crimson, #c41e3a)';this.style.background='color-mix(in srgb, var(--ds-crimson, #c41e3a) 8%, transparent)'" onmouseout="this.style.color='var(--text-muted)';this.style.background=''">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
