...
{{--
    AT-37 reusable per-user mailbox management block. Used by both
    Settings → Email Setup (the agency control centre) and the Admin → Users
    edit "Communication Capture" section — one component, not two code paths.

    Expects: $user (with ->commMailboxes loaded). Every form here posts to the
    settings.email-setup.* routes. The password field is WRITE-ONLY: it never
    renders a stored value; blank-on-edit keeps the current password.
--}}
@php
    $canReveal      = auth()->user()->hasPermission('reveal_mailbox_credential');
...
<div x-data="{ adding: false, editing: null }" class="space-y-3">
        ...
        <div class="rounded-md p-3" style="background: var(--surface-2, #f8fafc); border: 1px solid var(--border, #e5e7eb);">
            <div class="flex items-start justify-between gap-3">
                ...
                <div class="flex items-center gap-2 flex-shrink-0">
                    ...
                    @if($canReveal)
                        <form method="POST" action="{{ route('settings.email-setup.reveal', $mbx) }}" class="inline"
                    ...
                    @endif
                    <form method="POST" action="{{ route('settings.email-setup.destroy', $mbx) }}" class="inline"
            ...
            @if($canReveal && (int) $revealedId === (int) $mbx->id && $revealedPass !== null)
            ...
            <div x-show="editing === {{ $mbx->id }}" x-cloak class="mt-3 pt-3" style="border-top: 1px solid var(--border, #e5e7eb);">
                <form method="POST" action="{{ route('settings.email-setup.update', $mbx) }}" class="space-y-3">
                    @csrf @method('PUT')
    ...
    <div>
        ...
        <div x-show="adding" x-cloak class="rounded-md p-3 mt-1" style="background: var(--surface-2, #f8fafc); border: 1px solid var(--border, #e5e7eb);">
            <form method="POST" action="{{ route('settings.email-setup.store', $user) }}" class="space-y-3">
                @csrf
