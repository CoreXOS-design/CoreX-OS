            ...
            <ul class="space-y-2 mb-4">
                @foreach($links as $link)
                    <li class="flex items-start justify-between gap-2 text-xs">
                        <div class="min-w-0">
                            <span class="inline-block px-1.5 py-0.5 rounded bg-gray-100 text-gray-500 text-xs font-medium mr-1">
                                {{ ucfirst(str_replace('property24', 'Property24', $link->type)) }}
                            </span>
        ...
        <form method="POST" action="{{ route('presentations.upload', $presentation) }}"
            ...
            <div class="flex gap-2 items-center">
                <input type="file" name="document"
                       class="flex-1 text-xs text-gray-600 border border-gray-300 rounded px-2 py-1.5">
                <button type="submit"
