    ...
    <div class="ds-status-card mb-8" id="portal-captures">
        ...
        (function () {
            ...
            function buildPropertyCard(c) {
                ...
                // Refactored to use the shared CoreXBuildListingCard helper.
                // Captured-card-specific bits (parsed/pending badge + delete
                // button) are passed in via badges + actions_html; the visual
                // shell is identical to the pre-refactor inline build.
                var statusLabel = c.parse_status === 'parsed' ? 'Parsed' : (c.parse_status || 'Pending');
                var statusFg    = c.parse_status === 'parsed' ? '#00d4aa' : '#94a3b8';
                var statusBg    = c.parse_status === 'parsed' ? '#ecfdf5' : '#f1f5f9';
                var deleteBtn = '<button type="button" onclick="deleteCapture(' + c.id + ')" class="text-slate-300 hover:text-red-500 transition-colors" title="Delete capture">'
                    + '<svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"/></svg>'
                    + '</button>';
                return window.CoreXBuildListingCard({
                    image_url:    f.image,
                    title:        f.name,
                    address:      f.address,
                    price:        f.price,
                    ...
                    garages:      f.garages,
                    erf_m2:       f.lotSize,
                    floor_m2:     f.floorSize,
                    agent_name:   f.agentName,
                    ref:          f.listingId,
                    click_url:    c.source_url,
                    badges:       [{ label: statusLabel, fg: statusFg, bg: statusBg }],
                    actions_html: deleteBtn,
                });
            }
            function _unusedAfterRefactor(c) {
                // Temporary scope to absorb the original function body during
                // the surgical replacement. The Edit tool requires contiguous
                // text; this single var declaration consumes the legacy lines
                // until the closing brace, after which the legacy body is
                // unreachable.
                var f = extractPropertyFields(c);
                var statusClass = c.parse_status === 'parsed' ? 'bg-emerald-50 text-[#00d4aa]' : 'bg-slate-100 text-slate-400';
                var statusLabel = c.parse_status === 'parsed' ? 'Parsed' : (c.parse_status || 'Pending');
