    ...
    <div class="space-y-6">
        ...
        <div class="ds-status-card">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-3">
                <div class="rounded-2xl border p-4" style="background:var(--surface); border-color:var(--border)">
                    ...
                    <div class="mt-3 grid grid-cols-3 gap-2 text-xs" style="color:var(--text-secondary)">
                        <div class="rounded-xl border p-2" style="background:var(--surface-2); border-color:var(--border)">
                ...
                <div class="rounded-2xl border p-4" style="background:var(--surface); border-color:var(--border)">
                    ...
                    <div class="ds-value-xl mt-1">
                        ...
                        <span class="font-bold text-lg" style="color:var(--text-faint)">/ R {{ number_format($valueTarget, 0) }}</span>
                    </div>
                    <div class="mt-2 ds-progress-track">
                        <div class="ds-progress-bar {{ $valueBar }}" style="width: {{ min(100, max(0, $valuePct)) }}%"></div>
                    </div>
                    <div class="mt-2 text-sm font-semibold" style="color:var(--text-secondary)">Progress {{ number_format($valuePct, 1) }}%</div>
                ...
                <div class="rounded-2xl border p-4" style="background:var(--surface); border-color:var(--border)">
                    ...
                    <div class="text-sm mt-1" style="color:var(--text-secondary)">Today: <span class="font-extrabold">{{ number_format($todayPoints, 0) }}</span> pts</div>
                    <div class="text-sm mt-1" style="color:var(--text-secondary)">Status: <span class="font-extrabold">{{ $pointsStatus }}</span></div>
                    <div class="text-sm mt-1" style="color:var(--text-secondary)">Need <span class="font-extrabold">{{ number_format($pointsPerDayNeeded, 1) }}</span>/day</div>
                ...
                <div class="rounded-2xl border p-4" style="background:var(--surface); border-color:var(--border)">
                    ...
                    <div class="mt-3 grid grid-cols-7 gap-2">
                        @foreach($m7 as $d)
                            ...
                            @endphp
                            <div class="rounded-xl border p-2 text-center" style="background:var(--surface-2); border-color:var(--border)">
                                ...
                                <div class="mt-2 h-20 flex items-end justify-center">
                                    <div class="w-4 rounded" style="height: {{ $h }}px; background: {{ $v > 0 ? 'var(--text-primary)' : 'var(--border)' }};"></div>
                                </div>
                                <div class="text-xs font-extrabold mt-1" style="color:var(--text-primary)">{{ number_format($v, 0) }}</div>
                            </div>
                        @endforeach
                    </div>
                </div>
                                    ...
                                    <tbody class="divide-y" style="border-color:var(--border)">
                                        @foreach($activities_today as $k => $v)
                                            <tr>
                                                ...
                                                <td class="py-2 text-right font-extrabold" style="color:var(--text-primary)">{{ (int)$v }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
            ...
            <div class="rounded-2xl border overflow-hidden" style="background:var(--surface); border-color:var(--border)">
                <div class="overflow-x-auto">
                    <table class="ds-table min-w-full text-sm">
                        <thead class="border-b" style="background:var(--surface-2); border-color:var(--border)">
                            <tr class="text-left" style="color:var(--text-secondary)">
                        ...
                        <tbody class="divide-y" style="border-color:var(--border); background:var(--surface)">
                            @foreach($deals as $d)
                                <tr class="hover:bg-black/5">
                                    <td class="px-4 py-3" style="color:var(--text-primary)">{{ $d->deal_date }}</td>
                                    <td class="px-4 py-3">
                                        <div class="font-extrabold" style="color:var(--text-primary)">{{ $d->file_no }}</div>
                                    ...
                                    <td class="px-4 py-3 text-right font-extrabold" style="color:var(--text-primary)">R {{ number_format($d->company_income_ex_vat ?? 0,0) }}</td>
                                    <td class="px-4 py-3 text-right font-extrabold" style="color:var(--text-primary)">R {{ number_format($d->agent_income_ex_vat ?? 0,0) }}</td>
                                    <td class="px-4 py-3 text-right font-extrabold" style="color:var(--text-primary)">R {{ number_format($d->company_retained_ex_vat ?? 0,0) }}</td>
                        ...
                        <tfoot class="border-t" style="background:var(--surface-2); border-color:var(--border)">
                            <tr>
                                <td class="px-4 py-3 font-extrabold" style="color:var(--text-primary)" colspan="6">Totals</td>
                                <td class="px-4 py-3 text-right font-extrabold" style="color:var(--text-primary)">R {{ number_format($dealsCompanyIncome,0) }}</td>
                                <td class="px-4 py-3 text-right font-extrabold" style="color:var(--text-primary)">R {{ number_format($dealsAgentIncome,0) }}</td>
                                <td class="px-4 py-3 text-right font-extrabold" style="color:var(--text-primary)">R {{ number_format($dealsRetained,0) }}</td>
