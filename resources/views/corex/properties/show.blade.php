...
<div class="w-full h-full flex flex-col space-y-4 corex-props-v2"
    ...
    <div class="flex gap-5 items-stretch flex-1" style="min-height:0;">
        ...
        <aside x-show="!sbCollapsed"
               class="hidden lg:flex flex-col gap-3 flex-shrink-0" style="width:280px; min-height:0; overflow-y:auto;">
        ...
        {{-- RIGHT: tabs --}}
        <div class="flex-1 min-w-0 prop-tab-panel" data-scroll-region
    ...
    {{-- Tab bar (shared) — sticky at the top of the scrolling tab panel --}}
        ...
        <div x-show="activeTab === 'overview'" x-cloak class="p-6 space-y-6">
            ...
            @php
                // Resolved once so the @if gate below checks the same value
                // the <img src> uses — a missing original now correctly falls
                // through to the placeholder instead of an empty src
                // (2026-08-25, same root cause as the MIC photo fix).
                $coverImage   = $property->thumbFor($property->gallery_images_json[0] ?? ($property->dawn_images_json[0] ?? null));
...
{{-- Alpine components backing the shared syndication panel included above. --}}
