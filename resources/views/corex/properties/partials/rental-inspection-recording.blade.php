                                ...
                                <div class="flex-1 min-w-0 overflow-x-auto"
                                    ...
                                    <template x-for="photo in itemPhotosFor({{ $sectionJs }}, item)" :key="photo.id">
                                        <div class="relative rounded-md" style="display:inline-block; vertical-align:top; height:80px; max-height:80px; overflow:hidden; background:var(--surface-3); margin-right:0.375rem;"
                                             :style="photoUploader({{ $sectionJs }}).isSelected(photo.id) ? 'outline:2px solid var(--brand-icon,#0ea5e9);' : ''">
                                            {{-- Clicking the photo opens it (unchanged) — the
                                                 controls below are their own small hit
                                                 targets, @click.stop, so none of them also
                                                 open the photo. cc2 owns this strip's
                                                 height/sizing (R2) — nothing below touches
                                                 that inline sizing, only adds overlay
                                                 buttons inside the tile's existing
                                                 position:relative. --}}
