...
@php
    $tpsInput = 'width:100%; background:var(--surface); border:1px solid var(--border); color:var(--text-primary);';
    $tpsLabel = 'display:block; margin-bottom:0.25rem; color:var(--text-secondary);';
@endphp

<div>
    <label class="text-[10px] font-medium" style="{{ $tpsLabel }}" for="tps_sold_by_agency">Which agency sold it?</label>
    <input type="text" id="tps_sold_by_agency" name="sold_by_agency" maxlength="200"
           value="{{ old('sold_by_agency', $record?->sold_by_agency) }}"
           placeholder="e.g. Seeff Margate"
           class="rounded px-2 py-1 text-xs" style="{{ $tpsInput }}">
    @error('sold_by_agency')
        <p class="mt-1 text-[10px]" style="color:var(--ds-crimson, #c41e3a);">{{ $message }}</p>
    @enderror
</div>

<div class="grid grid-cols-2 gap-2">
    <div>
        <label class="text-[10px] font-medium" style="{{ $tpsLabel }}" for="tps_sold_price">Sold price (R)</label>
    ...
    <div>
        <label class="text-[10px] font-medium" style="{{ $tpsLabel }}" for="tps_sold_date">Sold date</label>
        <input type="date" id="tps_sold_date" name="sold_date" max="{{ now()->toDateString() }}"
               value="{{ old('sold_date', $record?->sold_date?->toDateString()) }}"
               class="rounded px-2 py-1 text-xs" style="{{ $tpsInput }} color-scheme: light dark;">
        @error('sold_date')
            <p class="mt-1 text-[10px]" style="color:var(--ds-crimson, #c41e3a);">{{ $message }}</p>
        @enderror
    </div>
...
<div>
    <label class="text-[10px] font-medium" style="{{ $tpsLabel }}" for="tps_loss_reason">Why did we lose it?</label>
    <select id="tps_loss_reason" name="loss_reason" class="rounded px-2 py-1 text-xs" style="{{ $tpsInput }}">
...
<div>
    <label class="text-[10px] font-medium" style="{{ $tpsLabel }}" for="tps_notes">Notes</label>
    <textarea id="tps_notes" name="notes" rows="2" maxlength="2000"
              placeholder="Anything worth remembering next time we pitch this street"
              class="rounded px-2 py-1 text-xs" style="{{ $tpsInput }}">{{ old('notes', $record?->notes) }}</textarea>
    @error('notes')
        <p class="mt-1 text-[10px]" style="color:var(--ds-crimson, #c41e3a);">{{ $message }}</p>
    @enderror
</div>
