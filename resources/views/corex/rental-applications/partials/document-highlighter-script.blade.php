...
function rentalDocumentHighlighter({ initialMarkedUpDocIds, currentUserId, currentUserName, currentUserRole, highlighters, reviewLocked } = {}) {
    return {
        ...
        // 2026-09-13 — pen rail labels, Johan (twice) + cc4's own agent
        // walk: an unlabelled colour circle means an agent has no idea
        // which pen is which until she hovers, and hover-only is not a
        // label. ONE shared font size for the whole picker set — including
        // the literal "Note" string, which has no backing highlighter
        // record but sits in the same rail and must read as the same
        // scale, not visibly smaller or larger — chosen as the LARGEST
        // size at which the single longest label still fits the rail's own
        // 40px label column (see document-highlighter-pages.blade.php's
        // own comment for where that number comes from — the rail itself
        // stays 44px, unchanged, per Johan's explicit "the document's
        // width is the whole point of this rebuild"). Computed once in
        // init() above, off real rendered pixel width (canvas
        // measureText — character COUNT alone doesn't work, "iiii" and
        // "MMMM" render at very different widths at the same font size).
        penLabelFontSizePx: 7,
        computePenLabelFontSize() {
            const labels = this.pickerHighlighters().map(h => h.label || '').filter(Boolean).concat(['Note']);
            if (!labels.length) return;
            const MAX_SIZE = 8, MIN_SIZE = 7, AVAILABLE_WIDTH_PX = 36; // 40px label column minus a couple px of breathing room each side
