...
<div class="px-4 py-3 flex items-center gap-3" style="border-bottom:1px solid var(--border);">
    ...
    <div class="flex-1 min-w-0">
        ...
        <div class="text-xs flex gap-2 mt-0.5" style="color:var(--text-muted);">
            ...
            @if($doc->uploader)
            <span>&middot; {{ $doc->uploader->name }}</span>
            @endif
        </div>
    </div>
    ...
    <div class="flex items-center gap-2 flex-shrink-0">
        ...
        {{-- AT-267 — hide download for an assistant with the toggle off (route is gated too). --}}
...
<div x-show="editing" x-cloak class="px-4 py-2" style="border-bottom:1px solid var(--border); background:var(--surface-2);">
    <form method="POST" action="{{ route('corex.contacts.documents.tag', [$contact, $doc]) }}" class="flex items-center gap-3">
        ...
        <select name="property_id" class="text-xs rounded border px-2 py-1" style="border-color:var(--border); background:var(--surface-2); color:var(--text-primary);">
            ...
            <option value="{{ $prop->id }}" {{ $doc->properties->contains('id', $prop->id) ? 'selected' : '' }}>{{ $propLabel }}</option>
            @endforeach
        </select>
