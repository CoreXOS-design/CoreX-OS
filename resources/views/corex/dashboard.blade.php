...
<div class="space-y-6">
    ...
    {{-- AT-352b — Supervised candidates' IN-PROGRESS documents (read-only walk-through) --}}
    @if(isset($candidateInProgressDocs) && $candidateInProgressDocs->count() > 0)
        <div class="corex-panel border-l-4" style="border-left-color: #0ea5e9;">
            <div class="corex-panel-header">
                <h3 class="corex-panel-title" style="color: #0369a1;">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 inline-block mr-1 -mt-0.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                    </svg>
                    CANDIDATE DOCUMENTS &mdash; IN PROGRESS (VIEW ONLY)
                </h3>
                <span class="text-xs font-medium px-2 py-0.5 rounded-full" style="background: #e0f2fe; color: #075985;">
                    {{ $candidateInProgressDocs->count() }} live
                </span>
            </div>
            <div class="corex-panel-body">
                <p class="text-xs mb-2" style="color: var(--text-muted);">Read-only — open a candidate's live document to walk the party through it. Not yet your turn to sign.</p>
                <div class="divide-y" style="border-color: var(--border-default);">
                    @foreach($candidateInProgressDocs as $doc)
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between py-3 gap-2 {{ !$loop->first ? 'pt-3' : '' }}">
                            <div class="flex-1 min-w-0">
                                <p class="font-semibold text-sm truncate" style="color: var(--text-primary);">
                                    {{ $doc->document->name ?? 'Untitled Document' }}
                            ...
                            <div class="flex-shrink-0">
                                <a href="{{ route('docuperfect.signatures.viewLive', $doc->document_id) }}"
                                   class="inline-flex items-center gap-1.5 px-4 py-2 rounded-md text-sm font-semibold text-white shadow transition-all duration-200 hover:opacity-90"
