...
@section('corex-content')
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-6">
        ...
        <div class="xl:col-span-2 corex-panel">
            <div class="corex-panel-header">
                <h3 class="corex-panel-title">Transaction Volume</h3>
                <span class="text-xs text-gray-500">Last 6 months</span>
            </div>
            <div class="corex-panel-body">
                <div class="corex-chart-container"
                     x-data
        ...
        <div class="corex-panel">
            <div class="corex-panel-header">
                <h3 class="corex-panel-title">Approval Queue</h3>
                <a href="{{ route('admin.deals') }}" class="text-xs text-[#00b4d8] font-medium hover:underline">View all</a>
            </div>
            <div class="corex-panel-body">
                @forelse($approvalQueue as $deal)
                    <div class="corex-queue-item">
                        <div>
                            <div class="corex-queue-label">{{ $deal->deal_no ?: ('Deal #' . $deal->id) }}</div>
                            <div class="corex-queue-sub">{{ Str::limit($deal->property_address, 30) }}</div>
                        </div>
                        @php
                            $badgeClass = match($deal->commission_status) {
                                'Granted' => 'corex-badge-green',
                                'Pending' => 'corex-badge-yellow',
                                'Declined' => 'corex-badge-red',
                                'Registered' => 'corex-badge-blue',
                                default => 'corex-badge-yellow',
                            };
                        @endphp
                        <span class="corex-badge {{ $badgeClass }}">{{ $deal->commission_status }}</span>
                    </div>
                @empty
    ...
    <div class="corex-panel">
        <div class="corex-panel-header">
            <h3 class="corex-panel-title">Recent Activity</h3>
        </div>
        <div class="corex-panel-body">
            @forelse($recentActivity as $log)
                <div class="corex-activity-item">
                    <div class="corex-activity-dot"></div>
                    <div>
                        <div class="corex-activity-text">
                        ...
                        <div class="corex-activity-time">{{ $log->created_at?->diffForHumans() ?? '' }}</div>
                    </div>
                </div>
            @empty
