...
<div class="w-full h-full flex flex-col gap-4">
    ...
    @endphp
        @foreach($captures as $tp)
            ...
            <div x-show="selected === 'tp-{{ $tp->id }}'" x-cloak class="flex-1 min-h-0 flex flex-col">
                ...
                <div class="flex-1 min-h-0 overflow-y-auto px-5 py-4 corex-brand-scroll">
    ...
    {{-- HEADER — same shape as the contact page header (corex.contacts._header,
         AT-336 / AT-393): a surface card, NOT a corex-page-banner. Two bands:
           · Identity row — the screen name LEFT, every filter control RIGHT, one line.
           · Facts strip  — six equal cells across the bottom, hairline rules from a
                            1px grid gap over a --border background.
         The facts are the queue's counts, which is why the (output-free) capture
         loops run ABOVE this block: they push rows/panels onto Blade stacks and
         tally $deedsCounts on the way; nothing renders until the frame below. --}}
    <div class="rounded-lg overflow-hidden flex-shrink-0" style="background:var(--surface); border:1px solid var(--border); box-shadow:0 1px 2px rgba(15,23,42,0.06);">
        ...
        <div class="px-5 py-3.5 flex flex-wrap items-center justify-between gap-x-4 gap-y-3">
            <div class="flex flex-wrap items-center gap-x-3 gap-y-2 min-w-0">
                <h1 class="text-2xl font-bold leading-tight" style="color: var(--text-primary);">Deeds Capture</h1>
                <span class="text-[11px] px-2 py-0.5 rounded-md font-semibold"
                      style="background:var(--surface-2); border:1px solid var(--border); color:var(--text-secondary);">
                    {{ $captures->total() + $tvaStandalone->count() }} waiting
                </span>
            </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-px"
            ...
            <div class="px-4 py-2.5 min-w-0" style="background:var(--surface-2);">
                <div class="text-[11px] uppercase tracking-widest font-semibold" style="color:var(--text-muted);">Waiting</div>
                <div class="text-sm truncate mt-0.5" style="color:var(--text-primary);">
                    {{ number_format($captures->total() + $tvaStandalone->count()) }}
                    @if($captures->hasPages())<span style="color:var(--text-muted);">· page {{ $captures->currentPage() }} of {{ $captures->lastPage() }}</span>@endif
                </div>
            </div>

            <div class="px-4 py-2.5 min-w-0" style="background:var(--surface-2);">
                <div class="text-[11px] uppercase tracking-widest font-semibold" style="color:var(--text-muted);">New to us</div>
                <div class="text-sm truncate mt-0.5" style="color: {{ $deedsCounts['new'] > 0 ? 'var(--ds-green, #059669)' : 'var(--text-muted)' }};">
                    {{ $deedsCounts['new'] > 0 ? number_format($deedsCounts['new']) : '—' }}
                </div>
            </div>

            <div class="px-4 py-2.5 min-w-0" style="background:var(--surface-2);">
                <div class="text-[11px] uppercase tracking-widest font-semibold" style="color:var(--text-muted);">Possible matches</div>
                <div class="text-sm truncate mt-0.5" style="color: {{ $deedsCounts['match'] > 0 ? 'var(--ds-amber, #f59e0b)' : 'var(--text-muted)' }};">
                    {{ $deedsCounts['match'] > 0 ? number_format($deedsCounts['match']) : '—' }}
                </div>
            </div>

            <div class="px-4 py-2.5 min-w-0" style="background:var(--surface-2);">
                <div class="text-[11px] uppercase tracking-widest font-semibold" style="color:var(--text-muted);">Need a look</div>
                <div class="text-sm truncate mt-0.5" style="color: {{ $deedsCounts['look'] > 0 ? 'var(--ds-crimson, #dc2626)' : 'var(--text-muted)' }};">
                    {{ $deedsCounts['look'] > 0 ? number_format($deedsCounts['look']) : '—' }}
                </div>
            </div>

            <div class="px-4 py-2.5 min-w-0" style="background:var(--surface-2);">
                <div class="text-[11px] uppercase tracking-widest font-semibold" style="color:var(--text-muted);">Virtual Agent numbers</div>
                <div class="text-sm truncate mt-0.5" style="color: {{ $deedsCounts['tva'] > 0 ? 'var(--brand-icon, #0ea5e9)' : 'var(--text-muted)' }};">
                    {{ $deedsCounts['tva'] > 0 ? number_format($deedsCounts['tva']) : '—' }}
                </div>
            </div>

            <div class="px-4 py-2.5 min-w-0" style="background:var(--surface-2);">
                <div class="text-[11px] uppercase tracking-widest font-semibold" style="color:var(--text-muted);">Showing</div>
                <div class="text-sm truncate mt-0.5" style="color:var(--text-primary);">
                    {{ ['own' => 'Own', 'branch' => 'Branch', 'all' => 'All'][$deedsScope] ?? 'Own' }}
                    @if($selectedAgent)<span style="color:var(--text-muted);">· {{ $selectedAgent->name }}</span>@elseif($filterAgentId === 'unassigned')<span style="color:var(--text-muted);">· Unattributed</span>@endif
                    @if($searchTerm !== '')<span style="color:var(--text-muted);">· “{{ $searchTerm }}”</span>@endif
                </div>
            </div>
        </div>
    </div>
    ...
    @else
        ...
        <div class="flex-1 min-h-0 flex flex-col lg:flex-row gap-3"
            ...
            <div class="rounded-md flex flex-col min-h-0 flex-shrink-0 w-full lg:w-[380px] max-h-[40vh] lg:max-h-none" style="background: var(--surface); border: 1px solid var(--border);">
                <div class="flex-1 min-h-0 overflow-y-auto corex-brand-scroll" data-scroll-region>
