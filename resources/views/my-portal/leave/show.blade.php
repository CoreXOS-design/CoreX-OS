...
<div class="-m-4 lg:-m-6">
    ...
    <div class="p-4 lg:p-6 max-w-3xl">
        ...
        <div class="p-4 mb-4" style="background:{{ $c[1] }}; border:1px solid {{ $c[0] }}25; border-radius:3px;">
        ...
        <div class="p-4" style="background:var(--surface-2, #f8fafc); border:1px solid var(--border, #e5e7eb); border-radius:3px;" x-data="{ showCancel: false }">
            ...
            <form method="POST" action="{{ route('my-portal.leave.cancel', $application) }}" x-show="showCancel" x-cloak class="mt-3 space-y-3">
                ...
                <div>
                    ...
                    <input type="text" name="cancellation_reason" maxlength="500" placeholder="e.g. Plans changed" class="w-full px-3 py-2 text-sm focus:outline-none" style="background:#fff; border:1px solid var(--border, #e5e7eb); color:var(--text-primary, #0f172a); border-radius:3px;">
                </div>
                <div class="flex gap-2">
                    <button type="submit" class="px-3 py-1.5 text-xs font-semibold text-white" style="background:#ef4444; border-radius:3px;" onclick="return confirm('Cancel this leave application?')">Confirm Cancel</button>
                    <button type="button" @click="showCancel = false" class="px-3 py-1.5 text-xs font-semibold" style="color:var(--text-secondary, #6b7280); border:1px solid var(--border, #e5e7eb); border-radius:3px; background:none; cursor:pointer;">Keep Application</button>
                </div>
            </form>
        </div>
        @endif
