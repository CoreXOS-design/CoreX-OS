...
<div class="w-full space-y-5"
    ...
    <div x-show="tab === 'tools'" x-cloak>
        ...
        {{-- Local Drafts (this device) — AT-165 POPIA control --}}
        <div class="rounded-md p-5 mt-5" style="background:var(--surface); border:1px solid var(--border);"
             x-data="{ busy: false, msg: '' }">
            <h3 class="text-base font-bold mb-1" style="color:var(--text-primary);">Local Drafts (this device)</h3>
            <p class="text-xs mb-4" style="color:var(--text-muted);">
                As you fill in long forms, CoreX saves your unsaved changes to <strong>this browser on this device</strong>
                so a dropped connection or a sleeping laptop never loses your work. Drafts stay only on this device,
                never leave it, and clear themselves when you save or after 7 days. Use this to wipe every draft stored
                for your account on this device right now.
            </p>
            <div class="flex items-center gap-3">
                <button type="button"
                        :disabled="busy"
                        @click="busy = true; msg = '';
                                (window.CoreX?.draft?.clearForCurrentUser() || Promise.resolve(0))
                                    .then(n => { msg = 'Cleared ' + n + ' local draft' + (n === 1 ? '' : 's') + ' from this device.'; })
                                    .catch(() => { msg = 'Could not clear local drafts.'; })
                                    .finally(() => { busy = false; });"
                        class="inline-flex items-center gap-2 rounded-md px-4 py-2 text-sm font-semibold transition-colors"
                    ...
                    <span x-text="busy ? 'Clearing…' : 'Clear my local drafts'"></span>
                </button>
                <span class="text-xs" style="color:var(--text-muted);" x-text="msg"></span>
            </div>
        </div>
