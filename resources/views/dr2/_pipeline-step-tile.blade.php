...
<div class="dr2-tile {{ $wide ? 'dr2-tile--wide' : '' }} {{ $variant === 'gate' ? 'dr2-tile--gate' : '' }} {{ $isDone ? 'dr2-tile--done' : '' }} {{ $isNa ? 'dr2-tile--na' : '' }} {{ $awaiting ? 'dr2-tile--warn' : '' }}"
    ...
    <div class="dr2-tile__btns">
        ...
        @elseif($isDone && !$locked)
            @permission('view_deals')
            <form method="POST" action="{{ route('deals-dr2.pipeline.step.reopen', [$deal, $s]) }}" onsubmit="return confirm('Reopen this step? It returns to Not started and downstream dates re-cascade.');">@csrf<input type="hidden" name="from" value="{{ $from ?? '' }}">
        ...
        @elseif($isNa && !$locked)
            @permission('view_deals')
            <form method="POST" action="{{ route('deals-dr2.pipeline.step.reinstate', [$deal, $s]) }}">@csrf<input type="hidden" name="from" value="{{ $from ?? '' }}">
        ...
        @if(!$locked)
            @permission('view_deals')
            <form method="POST" action="{{ route('deals-dr2.pipeline.step.remove', [$deal, $s]) }}" onsubmit="return confirm('Remove this step? It is archived, not deleted.');">@csrf<input type="hidden" name="from" value="{{ $from ?? '' }}">
    ...
    <template x-teleport="body"><div class="dr2-modal" x-show="done" x-cloak @keydown.escape.window="done=false">
        ...
        <div class="dr2-modal__card">
            ...
            <form method="POST" action="{{ route('deals-dr2.pipeline.step.complete', [$deal, $s]) }}">@csrf<input type="hidden" name="from" value="{{ $from ?? '' }}">
    ...
    <template x-teleport="body"><div class="dr2-modal" x-show="due" x-cloak @keydown.escape.window="due=false">
        ...
        <div class="dr2-modal__card">
            ...
            <form method="POST" action="{{ route('deals-dr2.pipeline.step.due', [$deal, $s]) }}">@csrf<input type="hidden" name="from" value="{{ $from ?? '' }}">
    ...
    <template x-teleport="body"><div class="dr2-modal" x-show="seq" x-cloak @keydown.escape.window="seq=false">
        ...
        <div class="dr2-modal__card">
            ...
            <form method="POST" action="{{ route('deals-dr2.pipeline.step.follows', [$deal, $s]) }}">@csrf<input type="hidden" name="from" value="{{ $from ?? '' }}">
    ...
    <template x-teleport="body"><div class="dr2-modal" x-show="na" x-cloak @keydown.escape.window="na=false">
        ...
        <div class="dr2-modal__card">
            ...
            <form method="POST" action="{{ route('deals-dr2.pipeline.step.na', [$deal, $s]) }}">@csrf<input type="hidden" name="from" value="{{ $from ?? '' }}">
    ...
    <template x-teleport="body"><div class="dr2-modal" x-show="cm" x-cloak @keydown.escape.window="cm=false">
        ...
        <div class="dr2-modal__card dr2-modal__card--wide">
            ...
            <form method="POST" action="{{ route('deals-dr2.pipeline.step.comment', [$deal, $s]) }}" class="dr2-modal__cmform">@csrf<input type="hidden" name="from" value="{{ $from ?? '' }}">
            ...
            @unless($locked)
            <form method="POST" action="{{ route('deals-dr2.documents.store', $deal) }}" enctype="multipart/form-data" class="dr2-modal__cmform">@csrf<input type="hidden" name="from" value="{{ $from ?? '' }}">
