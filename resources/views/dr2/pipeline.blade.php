    ...
    @else
        ...
        <div class="corex-card" style="margin-top:1rem;padding:.5rem;{{ $locked ? 'opacity:.72;filter:grayscale(.35);' : '' }}">
            @foreach($steps as $row)
                ...
                <div x-data="{ na:false, cm:false, due:false }" style="border-bottom:1px solid var(--border);padding:.65rem .5rem;{{ $row['na'] ? 'opacity:.6;' : '' }}">
                    <div style="display:flex;align-items:flex-start;gap:.6rem;">
                        ...
                        <div style="flex:1 1 auto;min-width:0;">
                            <div style="{{ $row['na'] ? 'text-decoration:line-through;' : '' }}">
                                ...
                                @if($s->is_custom)<span style="font-size:.7rem;color:var(--brand-icon);margin-left:.35rem;">+ custom</span>@endif
                            </div>
                            @if($row['blocked'])<div style="font-size:.75rem;color:var(--text-muted);">{{ $row['blocked'] }}</div>@endif
                            @if($row['na'] && $s->na_reason)<div style="font-size:.75rem;color:var(--text-muted);">Excused: {{ $s->na_reason }}</div>@endif
                        </div>
                        <div style="flex:0 0 auto;text-align:right;font-size:.8rem;">
                            ...
                            <div style="color:var(--text-muted);margin-top:.15rem;">{{ $s->due_date ? \Illuminate\Support\Carbon::parse($s->due_date)->format('d M Y') : '—' }}</div>
                        </div>
                    </div>
                    ...
                    <div style="display:flex;gap:.4rem;flex-wrap:wrap;margin-top:.4rem;padding-left:1.3rem;">
                        ...
                        @if($s->status === 'active')
                            ...
                            <form method="POST" action="{{ route('deals-dr2.pipeline.step.complete', [$deal, $s]) }}">@csrf
                                <button type="submit" class="corex-btn-outline" style="padding:.2rem .6rem;font-size:.78rem;">Mark complete</button>
                            </form>
                            @endpermission
                        @endif
                        @unless($terminal)
            ...
            <div x-data="{ rm:false }" style="padding:.65rem .5rem;border-top:2px solid var(--border);">
