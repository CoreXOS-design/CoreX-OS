...
<div class="w-full space-y-5">
    ...
    <div class="rounded-md px-6 py-5 corex-page-banner">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div>
                <h1 class="text-base font-bold leading-tight" style="color: var(--text-primary);">Deal Register</h1>
                <p class="text-xs" style="color: var(--text-muted);">Track sales, commission and settlement across the agency.</p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                @permission('deals.create')
                <a href="{{ route('deals-dr2.create') }}" class="corex-btn-primary text-xs inline-flex items-center gap-2">
                                ...
                                @foreach($paidNotSettledDeals as $d)
                                    <tr style="border-top: 1px solid var(--border);">
                                        ...
                                        <td class="px-4 py-2.5">
                                            <a href="{{ route('deals-dr2.settle', $d) }}" class="corex-btn-primary text-xs px-3 py-1.5">
                                                Open settlement
                                            </a>
                                        </td>
                                    </tr>
    ...
    <div class="rounded-md px-4 py-3" style="background: var(--surface); border: 1px solid var(--border);">
        <form method="GET" action="{{ route('deals-dr2.index') }}" class="flex flex-wrap items-center gap-3">
            ...
            <a href="{{ route('deals-dr2.index') }}" class="text-xs underline transition-all duration-300" style="color: var(--text-muted);">Clear</a>
            @endif
                        ...
                        @foreach($deals as $deal)
                            ...
                            <tr>
                                <td class="px-3 py-2.5">
                                    <a href="{{ route('deals-dr2.edit', $deal) }}" class="ds-agent-link font-bold">{{ $deal->deal_no }}</a>
                                ...
                                <td class="px-2 py-2.5 text-center relative" x-data="{ open: false }">
                                    ...
                                    <div x-show="open" @click.outside="open = false" x-cloak
                                        ...
                                        <form method="POST" action="{{ route('deals-dr2.quickUpdate', $deal) }}">
                                            @csrf
                                ...
                                <td class="px-3 py-2.5 text-right">
                                    <div class="flex items-center justify-end gap-1.5">
                                        ...
                                        {{-- Phase 4 — an existing pipeline opens on the agent's remembered view (timeline|list).
                                             "Pipeline" shows once steps exist — from an attached template OR composed from the
                                             Deal Structure tab (composable deals have no template_id, so key off the step count). --}}
                                        @php $hasPipeline = ($deal->pipeline_steps_count ?? 0) > 0 || $deal->deal_pipeline_template_id; @endphp
                                        <a href="{{ route($hasPipeline ? 'deals-dr2.pipeline.view' : 'deals-dr2.pipeline', $deal) }}" class="corex-btn-outline text-xs px-2 py-1">{{ $hasPipeline ? 'Pipeline' : 'Attach' }}</a>
                                        @endpermission
                                        @permission('deals.edit')
                                        <a href="{{ route('deals-dr2.edit', $deal) }}" class="corex-btn-outline text-xs px-2 py-1">Edit</a>
                                        @endpermission
                                        @permission('settle_deals')
                                            <a href="{{ route('deals-dr2.settle', $deal) }}" class="corex-btn-primary text-xs px-2 py-1">Pay</a>
                                        @endpermission
                                    </div>
                                </td>
                            </tr>
                        ...
                        @if($deals->isEmpty())
                            <tr>
                                <td colspan="{{ ($branchIdContext ?? 0) > 0 ? 9 : 8 }}" class="px-6 py-12 text-center">
                                    ...
                                    @permission('deals.create')
                                    <a href="{{ route('deals-dr2.create') }}" class="corex-btn-primary text-sm">+ Add Deal</a>
                                    @endpermission
                                </td>
                            </tr>
                        @endif
