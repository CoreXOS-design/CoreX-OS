{{--
    The ONE shared "dead link, but make it a real page" template (Johan,
    2026-08-25 — "the link should be a rich engaging page, not a dead-looking
    screen"). Same visual standard as shared/match-expired.blade.php (Johan
    pointed at that page three times as the reference) — agency branding, the
    agent's own card, a couple of that agent's newest listings, a couple of
    the agency's newest, every section absent (not empty) when there's
    nothing to show.

    Every call site supplies its OWN $eyebrow/$headline/$lede/$primaryAction —
    that is the "per-context content" half of the brief. The markup below is
    the shared half.

    PRIVACY (non-negotiable, same standard as match-expired.blade.php):
    never reveal WHY a link died, never name a client, never show a property
    address here. $headline/$lede are supplied by the calling controller —
    that controller is responsible for keeping them generic-but-warm, not
    this view.

    Required: $eyebrow (string), $headline (string), $lede (string),
      $agency (?Agency), $agent (?User), $agentCardUrl (?string),
      $agentListings (Collection<Property>), $agencyListings (Collection<Property>),
      $agencyStockUrl (?string), $fallbackPhone (?string), $fallbackEmail (?string)
    Optional: $primaryAction (?array{label:string,url:string}),
...
<head>
    ...
    <title>{{ !empty($agency) ? $agency->name . ' — ' : '' }}{{ $headline }}</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .headline { font-weight: 800; font-size: clamp(1.85rem, 5vw, 3rem); line-height: 1.06; letter-spacing: -0.02em; margin: 0 0 1rem; max-width: 20ch; color: #ffffff; }
...
<body>
    ...
    {{-- 1. Band — agency identity + the headline for THIS context. --}}
    <div class="band">
        <div class="band-inner">
            <div class="logo-row">
                ...
                @else
                    <div class="text-lg font-bold tracking-tight" style="color:#fff;">{{ $agency->name ?? 'CoreX' }}</div>
                @endif
            </div>
            ...
            @if($primaryAction || $secondaryAction ?? null)
            <div class="band-actions">
                @if($primaryAction)
                    <a href="{{ $primaryAction['url'] }}" class="btn btn-invite">{{ $primaryAction['label'] }}</a>
                @endif
                @if($secondaryAction ?? null)
                    <a href="{{ $secondaryAction['url'] }}" class="btn btn-ghost">{{ $secondaryAction['label'] }}</a>
                @endif
            </div>
            @endif
        </div>
    ...
    {{-- 2. Agent row, overlapping the band. Absent when no active agent resolved. --}}
    @if($agent)
    <div class="lift">
    ...
    {{-- 3. "My newest stock" — absent (not empty-state) when the agent has no listings of their own. --}}
