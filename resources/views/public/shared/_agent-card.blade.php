{{--
    ...
    Agency-branded, token-driven (var(--token,#fallback)) so it drops onto any
    tokenised public page (buyer portal, seller page …). Build nothing duplicate.
    ...
    Relies on host :root tokens: --surface, --border, --text-*, --brand-default,
    --brand-button (all with in-partial fallbacks). Fully null-safe; never 500s.
--}}
@php
...
@if(!empty($agent))
<div style="background: var(--surface,#fff); border: 1px solid var(--border,rgba(0,0,0,.08)); border-radius: 10px; padding: 1.25rem;">
    <div style="font-size:.6875rem; font-weight:700; text-transform:uppercase; letter-spacing:.06em; color: var(--text-muted,#9ca3af); margin-bottom:.875rem;">
        {{ $heading }}
    </div>
    ...
    <div style="display:grid; grid-template-columns:1fr 1fr; gap:.5rem; margin-top:1rem;">
        @if($agentPhone)
            <a href="tel:{{ $agentPhone }}"
               style="display:flex; align-items:center; justify-content:center; gap:.4rem; min-height:44px; padding:.625rem; border-radius:8px; font-size:.875rem; font-weight:600; color:#fff; background: var(--brand-button,#00b4d8);">
        ...
        @if($waDigits)
            <a href="https://wa.me/{{ $waDigits }}" target="_blank" rel="noopener"
               style="display:flex; align-items:center; justify-content:center; gap:.4rem; min-height:44px; padding:.625rem; border-radius:8px; font-size:.875rem; font-weight:600; color:#fff; background:#25D366;">
        ...
        @if($agent->email)
            <a href="mailto:{{ $agent->email }}"
               style="grid-column:1 / -1; display:flex; align-items:center; justify-content:center; gap:.4rem; min-height:44px; padding:.625rem; border-radius:8px; font-size:.875rem; font-weight:600; color: var(--text-secondary,#4b5563); background: var(--surface,#fff); border:1px solid var(--border,rgba(0,0,0,.1));">
