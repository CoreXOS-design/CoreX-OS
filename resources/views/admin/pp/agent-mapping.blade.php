...
<div class="w-full space-y-5">
    ...
    <div class="rounded-md overflow-hidden" style="background:var(--surface); border:1px solid var(--border);">
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm pp-agent-table">
                <thead>
                    <tr style="background:var(--surface-2);">
                        <th class="text-left px-4 py-2.5 font-semibold uppercase tracking-wider text-xs" style="color:var(--text-muted);">Agent</th>
                        <th class="text-left px-4 py-2.5 font-semibold uppercase tracking-wider text-xs" style="color:var(--text-muted);">Email</th>
                        <th class="text-left px-4 py-2.5 font-semibold uppercase tracking-wider text-xs" style="color:var(--text-muted);">Role</th>
                        <th class="text-left px-4 py-2.5 font-semibold uppercase tracking-wider text-xs" style="color:var(--text-muted);">External Ref</th>
                        <th class="text-left px-4 py-2.5 font-semibold uppercase tracking-wider text-xs" style="color:var(--text-muted);">PP Status</th>
                        <th class="text-right px-4 py-2.5 font-semibold uppercase tracking-wider text-xs" style="color:var(--text-muted);">Action</th>
                    </tr>
                </thead>
                @forelse($agents as $a)
                    <tbody x-data="ppAgentRow({
                        ...
                        <tr class="pp-agent-main-row">
                            <td class="px-4 py-3 font-medium" style="color:var(--text-primary);">
                                {{ $a->name }}
                                @unless($a->is_active)
                                    <span class="ds-badge ds-badge-default ml-1.5">Inactive</span>
                                @endunless
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <span class="ds-badge" :class="badgeColor.cls" x-text="badgeColor.label"></span>
                            </td>
                            <td class="px-4 py-3 text-right">
                                <button type="button" @click="open = !open" class="corex-btn-outline corex-btn-xs">
                            ...
                            <td colspan="6" class="px-4 py-4" style="background:var(--surface-2);">
                                <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 max-w-4xl">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" x-model="externalRef" maxlength="100"
                                               class="pp-agent-input w-full rounded-md px-3 py-2.5 text-sm outline-none transition-colors"
                                    ...
                                    <div>
                                        ...
                                        <input type="text" x-model="ppEncryptedId" placeholder="Leave blank unless provided by PP" maxlength="500"
                                               class="pp-agent-input w-full rounded-md px-3 py-2.5 text-sm outline-none transition-colors"
                                               style="background:var(--surface); border:1px solid var(--border); color:var(--text-primary);">
                                        <p class="text-[11px] mt-1.5" style="color:var(--ds-amber,#f59e0b);">
                                ...
                                <div class="mt-4 flex flex-wrap gap-3">
                                    ...
                                    <button type="button" @click="deactivateAgent()" :disabled="deactivating"
                                            class="corex-btn-outline disabled:opacity-40 disabled:cursor-not-allowed"
                                            style="color:var(--ds-crimson); border-color:color-mix(in srgb, var(--ds-crimson) 30%, transparent); background:color-mix(in srgb, var(--ds-crimson) 8%, transparent);">
                                ...
                                <p x-show="updateMsg" x-cloak class="mt-2 text-xs font-medium"
                                   :style="updateOk ? 'color:var(--ds-green,#059669)' : 'color:var(--ds-crimson)'" x-text="updateMsg"></p>
                                <p x-show="syncMsg" x-cloak class="mt-2 text-xs font-medium"
                                   :style="syncOk ? 'color:var(--ds-green,#059669)' : 'color:var(--ds-crimson)'" x-text="syncMsg"></p>
                                <p x-show="deactivateMsg" x-cloak class="mt-2 text-xs font-medium"
                                   :style="deactivateOk ? 'color:var(--ds-green,#059669)' : 'color:var(--ds-crimson)'" x-text="deactivateMsg"></p>
                ...
                @empty
                    <tbody>
                        <tr><td colspan="6" class="px-4 py-12 text-center text-sm" style="color:var(--text-muted);">No agents in this agency.</td></tr>
                    </tbody>
                @endforelse
            </table>
        </div>
...
{{-- Component-level styling: this table uses a per-agent <tbody> group with an
     expandable editor row, so .ds-table's :nth-child(even) zebra rule can't be
     used (it would stripe the editor row). Scope the group borders + row hover
     + input focus here instead. All values come from design tokens. --}}
<style>
    .pp-agent-table > tbody { border-top: 1px solid var(--border); }
    .pp-agent-table > tbody:first-of-type { border-top: none; }
...
window.ppAgentRow = function (cfg) {
    return {
        ...
        get badgeColor() {
            if (this.updateOk === false) return { cls: 'ds-badge-danger', label: 'Error' };
            if (this.ppUniqueAgentId) return { cls: 'ds-badge-success', label: 'Claimed' };
            return { cls: 'ds-badge-default', label: 'Default' };
        },
