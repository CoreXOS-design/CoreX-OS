...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
    ...
    <form method="POST"
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 space-y-4">
            ...
            {{-- ── Property24 API Credentials (per-agency) ── --}}
            @if($agency)
            <div class="pt-4 border-t border-slate-200">
                <div class="text-sm font-bold uppercase tracking-wider mb-1" style="color:var(--brand-primary, #0b2a4a);">Property24 API Credentials</div>
                <p class="text-xs text-slate-400 mb-3">Used to authenticate against the Property24 Listing Service. Leave blank to use the global default from <code class="text-[10px]">.env</code>. The system uses these to fetch the suburb list (autofills on property forms) and to publish listings.</p>

                <div class="flex items-center gap-3 mb-4">
                    <input type="hidden" name="p24_enabled" value="0">
                    <input type="checkbox" name="p24_enabled" value="1" id="p24_enabled"
                           {{ old('p24_enabled', $agency?->p24_enabled ?? false) ? 'checked' : '' }}
                           class="w-4 h-4 rounded border-slate-300 cursor-pointer"
                           style="accent-color:var(--brand-icon, #0ea5e9);">
                    <label for="p24_enabled" class="text-sm font-medium cursor-pointer" style="color:var(--brand-default, #0b2a4a);">Enable Property24 integration for this agency</label>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold mb-1" style="color:var(--brand-primary, #0b2a4a);">Username</label>
                    ...
                    <div>
                        <label class="block text-sm font-semibold mb-1" style="color:var(--brand-primary, #0b2a4a);">Password</label>
                    ...
                    <div>
                        <label class="block text-sm font-semibold mb-1" style="color:var(--brand-primary, #0b2a4a);">User Group ID</label>
                ...
                {{-- Status + actions --}}
                <div class="mt-4 rounded-xl bg-slate-50 border border-slate-200 p-4" x-data="p24Actions({
                        testUrl: '{{ route('agencies.p24.test', $agency) }}',
                        refreshUrl: '{{ route('agencies.p24.refresh', $agency) }}',
                        csrf: '{{ csrf_token() }}',
                    })">
                    <div class="flex flex-wrap items-center justify-between gap-3">
                        <div class="text-xs space-y-1">
                            @if($agency->p24_locations_synced_at)
                                <div class="text-slate-600"><span class="font-semibold">Last synced:</span> {{ $agency->p24_locations_synced_at->diffForHumans() }} ({{ $agency->p24_locations_synced_at->format('Y-m-d H:i') }})</div>
                            @else
                                <div class="text-slate-500 italic">Never synced.</div>
                            @endif
                            @if($agency->p24_last_sync_error)
                                <div class="text-red-700 break-all"><span class="font-semibold">Last error:</span> {{ Str::limit($agency->p24_last_sync_error, 300) }}</div>
                            @endif
                            <template x-if="message">
                                <div :class="ok ? 'text-emerald-700' : 'text-red-700'" class="font-medium" x-text="message"></div>
                            </template>
                        </div>
                        <div class="flex gap-2">
                            <button type="button" @click="test()"
                                    :disabled="busy"
                                    class="px-3 py-1.5 rounded-lg text-xs font-semibold border border-slate-300 text-slate-700 bg-white hover:bg-slate-100 disabled:opacity-60">
                    ...
                    <p class="text-[11px] text-slate-400 mt-3">Saving this form with changed credentials automatically triggers a sync. Sync also runs on the 1st of every month at 02:00. Refresh manually if Property24 has added/removed suburbs since.</p>
                </div>
            </div>
