...
{{-- AT-173 — System Developer → Media Encryption. Read-only status of encryption
     ...
     Card style matches the plain single-tone panel used across the app (e.g.
     properties/reports/lost-to-competitors.blade.php) — an inline h2, no boxed
     title-bar/divider, no brand-icon blue on section labels. That treatment is
     reserved for real accents (links, the status pill below), not decoration. --}}
...
<div class="w-full space-y-5">
    ...
    <div class="rounded-md p-4" style="{{ $panel }}">
        <h2 class="text-sm font-semibold mb-3" style="color: var(--text-primary);">Status &amp; Health</h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
            ...
            <div class="{{ $cell }}"><span class="{{ $lbl }}" style="{{ $lblStyle }}">Algorithm</span><span class="{{ $val }}" style="color: var(--text-primary);">{{ $algorithm }} (authenticated)</span></div>
        </div>
    </div>

    {{-- Coverage --}}
    <div class="rounded-md p-4" style="{{ $panel }}">
        <h2 class="text-sm font-semibold mb-3" style="color: var(--text-primary);">What is encrypted</h2>
    ...
    {{-- Migration --}}
    <div class="rounded-md p-4" style="{{ $panel }}">
        <h2 class="text-sm font-semibold mb-2" style="color: var(--text-primary);">Encrypt existing files (backfill)</h2>
