...
<div class="-m-4 lg:-m-6" x-data="{ showFinalise: false }">
    ...
    <div class="p-4 lg:p-6">
        ...
        {{-- Run header card --}}
        <div class="flex flex-wrap items-center gap-4 mb-4 p-4" style="background:var(--surface-2, #f8fafc); border:1px solid var(--border, #e5e7eb); border-radius:3px;">
        ...
        @if($run->isDraft())
        <div class="mt-6 p-4" style="background:var(--surface-2, #f8fafc); border:1px solid var(--border, #e5e7eb); border-radius:3px;" x-data="{ showCancel: false }">
            ...
            <form method="POST" action="{{ route('payroll.runs.cancel', $run) }}" x-show="showCancel" x-cloak class="mt-3 space-y-3">
                ...
                <div class="flex gap-2">
                    <button type="submit" class="px-3 py-1.5 text-xs font-semibold text-white" style="background:#ef4444; border-radius:3px;" onclick="return confirm('This will cancel the run and soft-delete all draft payslips. Continue?')">Cancel Run</button>
                    <button type="button" @click="showCancel = false" class="px-3 py-1.5 text-xs font-semibold" style="color:var(--text-secondary, #6b7280); border:1px solid var(--border, #e5e7eb); border-radius:3px; background:none; cursor:pointer;">Keep Draft</button>
                </div>
            </form>
        </div>
        @endif
