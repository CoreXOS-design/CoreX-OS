                                        ...
                                        <tr>
                                            ...
                                            <td class="px-4 py-3">
                                                <div class="flex items-center gap-2">
                                                    <div class="w-7 h-7 rounded-full flex items-center justify-center text-[0.6875rem] font-bold text-white flex-shrink-0" style="background: var(--brand-default);">{{ strtoupper(substr($emp->user->name ?? '?', 0, 1)) }}</div>
                                                    <div class="min-w-0">
                                                        <p class="text-xs font-semibold truncate" style="color: var(--text-primary);">
                                                            {{ $emp->user->name ?? 'Unknown' }}
                                                            @if(!empty($emp->is_leaver))
                                                                <span class="ml-1 inline-block px-1.5 py-0.5 rounded text-[0.625rem] font-bold" style="background: color-mix(in srgb, var(--ds-crimson) 12%, transparent); color: var(--ds-crimson);" title="Terminates {{ $emp->termination_date?->format('d M Y') }} — final payslip, basic pro-rated">FINAL · leaves {{ $emp->termination_date?->format('d M') }}</span>
                                                            @endif
                                                        </p>
