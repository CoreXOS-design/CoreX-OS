...
<div class="w-full space-y-5">
    ...
    <div class="rounded-md px-4 py-3" style="background: var(--surface); border: 1px solid var(--border);">
        <form method="GET" action="{{ route('payroll.earning-types.index') }}" class="flex flex-wrap items-center gap-3">
    ...
    <div class="rounded-md py-12 px-6 text-center" style="background: var(--surface); border: 1px solid var(--border);">
        ...
        <h3 class="text-base font-semibold mb-1" style="color: var(--text-primary);">No earning types {{ $q ? 'match your search' : 'yet' }}</h3>
        <p class="text-sm mb-4" style="color: var(--text-muted);">{{ $q ? 'Try a different search term, or clear your filters.' : 'Add your first earning type to configure agency payroll categories.' }}</p>
        @if($q)
            <a href="{{ route('payroll.earning-types.index') }}" class="corex-btn-outline text-sm">Clear filters</a>
        @else
            <a href="{{ route('payroll.earning-types.create') }}" class="corex-btn-primary text-sm">Add Earning Type</a>
        @endif
    </div>
    @else
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm ds-table">
                <colgroup>
                    ...
                    <col style="width:64px;">{{-- Tax --}}
                    <col style="width:64px;">{{-- FB --}}
                    <col style="width:64px;">{{-- UIF --}}
                    <col style="width:64px;">{{-- SDL --}}
                ...
                <thead>
                    <tr style="background: var(--surface-2);">
                        ...
                        <th class="text-center px-2 py-2.5 text-xs font-semibold uppercase tracking-wider" style="color: var(--text-muted);" title="Taxable — subject to PAYE">Tax</th>
                        <th class="text-center px-2 py-2.5 text-xs font-semibold uppercase tracking-wider" style="color: var(--text-muted);" title="Fringe Benefit">FB</th>
                        <th class="text-center px-2 py-2.5 text-xs font-semibold uppercase tracking-wider" style="color: var(--text-muted);" title="Affects UIF remuneration">UIF</th>
                        <th class="text-center px-2 py-2.5 text-xs font-semibold uppercase tracking-wider" style="color: var(--text-muted);" title="Affects SDL remuneration">SDL</th>
                ...
                <tbody>
                    ...
                    <tr style="{{ !$type->is_active ? 'opacity:0.55;' : '' }}">
                        ...
                        <td class="px-2 py-3 text-center">
                            @if($type->is_taxable)
                                <svg class="w-4 h-4 mx-auto" style="color: var(--brand-icon);" fill="none" stroke="currentColor" viewBox="0 0 24 24" title="Taxable"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                            @else
                        ...
                        <td class="px-2 py-3 text-center">
                            @if($type->is_fringe_benefit)
                                <svg class="w-4 h-4 mx-auto" style="color: var(--brand-icon);" fill="none" stroke="currentColor" viewBox="0 0 24 24" title="Fringe benefit"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                            @else
                                <span class="text-xs" style="color: var(--text-muted);">—</span>
                            @endif
                        </td>
                        <td class="px-2 py-3 text-center">
                            @if($type->affects_uif_remuneration)
                                <svg class="w-4 h-4 mx-auto" style="color: var(--brand-icon);" fill="none" stroke="currentColor" viewBox="0 0 24 24" title="Affects UIF"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                            @else
                                <span class="text-xs" style="color: var(--text-muted);">—</span>
                            @endif
                        </td>
                        <td class="px-2 py-3 text-center">
                            @if($type->affects_sdl_remuneration)
                                <svg class="w-4 h-4 mx-auto" style="color: var(--brand-icon);" fill="none" stroke="currentColor" viewBox="0 0 24 24" title="Affects SDL"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                            @else
                                <span class="text-xs" style="color: var(--text-muted);">—</span>
                            @endif
                        </td>
                            ...
                            <div class="flex items-center justify-end gap-3">
                                ...
                                @if(!$type->is_system)
                                    <form method="POST" action="{{ route('payroll.earning-types.destroy', $type) }}" class="inline"
                                          onsubmit="return confirm('Delete this earning type? It will be archived and can be recovered by an admin.')">
                                        @csrf
                                        @method('DELETE')
