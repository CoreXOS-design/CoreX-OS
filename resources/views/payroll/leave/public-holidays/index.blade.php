    ...
    <x-page-header title="Public Holidays" :flush="true">
        <x-slot:actions>
            <form method="GET" action="{{ route('payroll.leave.public-holidays.index') }}" class="inline-flex items-center gap-2">
                <select name="year" onchange="this.form.submit()" class="px-3 py-2 text-xs focus:outline-none" style="background:var(--surface-2, #f8fafc); border:1px solid var(--border, #e5e7eb); color:var(--text-primary, #0f172a); border-radius:3px;">
                    @foreach($years as $y)
                        <option value="{{ $y }}" {{ $year == $y ? 'selected' : '' }}>{{ $y }}</option>
                    @endforeach
                </select>
            </form>
            <a href="{{ route('payroll.leave.public-holidays.create') }}" class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-white transition" style="background:#00d4aa; border-radius:3px;">
                    ...
                    <tbody>
                        ...
                        <tr style="border-bottom:1px solid var(--border, #e5e7eb);">
                            ...
                            <td class="px-3 py-2.5 text-center">
                                @if($h->is_movable)
                                    <span class="px-1.5 py-0.5 text-[10px] font-semibold" style="background:rgba(234,179,8,0.1); color:#eab308; border-radius:3px;">Moveable</span>
                                @else
                                    <span class="px-1.5 py-0.5 text-[10px] font-semibold" style="background:rgba(148,163,184,0.15); color:#94a3b8; border-radius:3px;">Fixed</span>
                                @endif
                            </td>
                            <td class="px-3 py-2.5 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('payroll.leave.public-holidays.edit', $h) }}" class="text-xs font-semibold" style="color:#00d4aa;">Edit</a>
                                    <form method="POST" action="{{ route('payroll.leave.public-holidays.destroy', $h) }}" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-xs font-semibold" style="color:#ef4444; background:none; border:none; cursor:pointer;" onclick="return confirm('Delete this holiday?')">Delete</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
