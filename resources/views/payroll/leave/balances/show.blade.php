    ...
    <x-page-header title="{{ $employee->user->name }} — Leave Balances" :back-route="route('payroll.leave.balances.index')" back-label="Balances" :flush="true">
        <x-slot:actions>
            <form method="POST" action="{{ route('payroll.leave.balances.recalculate', $employee) }}" class="inline">
                @csrf
                <button type="submit" class="px-3 py-2 text-xs font-semibold transition" style="color:var(--text-primary, #0f172a); border:1px solid var(--border, #e5e7eb); border-radius:3px; background:none; cursor:pointer;" onclick="return confirm('Recalculate all balances from transaction ledger?')">Recalculate</button>
            </form>
        </x-slot:actions>
    </x-page-header>
            ...
            <div class="lg:w-1/3 space-y-4">
                ...
                <div class="p-4" style="background:var(--surface-2, #f8fafc); border:1px solid var(--border, #e5e7eb); border-radius:3px;">
                    ...
                    @if($takeOn->isComplete())
                        <span class="px-1.5 py-0.5 text-[10px] font-semibold" style="background:rgba(0,212,170,0.1); color:#00d4aa; border-radius:3px;">Completed {{ $takeOn->completed_at->format('d M Y') }}</span>
                    @else
                        <span class="px-1.5 py-0.5 text-[10px] font-semibold" style="background:rgba(234,179,8,0.1); color:#eab308; border-radius:3px;">In Progress ({{ $takeOn->progressPercentage() }}%)</span>
                    @endif
                </div>
                @endif
            </div>
            ...
            <div class="lg:w-2/3" x-data="{ activeType: {{ $leaveTypes->first()?->id ?? 0 }} }">
                ...
                <div x-show="activeType === {{ $type->id }}" x-cloak>
                    ...
                    <div x-data="{ showAdjust: false }" class="mt-3">
                        ...
                        <form method="POST" action="{{ route('payroll.leave.balances.adjust', $employee) }}" x-show="showAdjust" x-cloak class="mt-2 p-3 space-y-3" style="background:rgba(0,212,170,0.03); border:1px solid rgba(0,212,170,0.15); border-radius:3px;">
                            @csrf
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                <div>
                                    ...
                                    <input type="number" name="days_delta" step="0.5" required class="w-full px-2 py-1.5 text-xs focus:outline-none" style="background:#fff; border:1px solid var(--border, #e5e7eb); border-radius:3px; color:var(--text-primary, #0f172a);">
                                </div>
                                <div>
                                    <label class="block text-[10px] font-semibold mb-0.5" style="color:var(--text-secondary, #6b7280);">Effective Date</label>
                                    <input type="date" name="effective_date" value="{{ date('Y-m-d') }}" class="w-full px-2 py-1.5 text-xs focus:outline-none" style="background:#fff; border:1px solid var(--border, #e5e7eb); border-radius:3px; color:var(--text-primary, #0f172a);">
                                </div>
                                <div class="sm:col-span-3">
                                    <label class="block text-[10px] font-semibold mb-0.5" style="color:var(--text-secondary, #6b7280);">Reason (min 10 chars)</label>
                                    <textarea name="reason" required minlength="10" rows="2" class="w-full px-2 py-1.5 text-xs focus:outline-none" style="background:#fff; border:1px solid var(--border, #e5e7eb); border-radius:3px; color:var(--text-primary, #0f172a);" placeholder="Explain why this adjustment is necessary..."></textarea>
                                </div>
                            </div>
                            <div class="flex gap-2">
                                <button type="submit" class="px-3 py-1.5 text-xs font-semibold text-white" style="background:#00d4aa; border-radius:3px;">Save Adjustment</button>
                                <button type="button" @click="showAdjust = false" class="px-3 py-1.5 text-xs font-semibold" style="color:var(--text-secondary, #6b7280); border:1px solid var(--border, #e5e7eb); border-radius:3px; background:none; cursor:pointer;">Cancel</button>
                            </div>
                        </form>
                    </div>
                    @endpermission
