...
<div class="w-full space-y-5">
    ...
    <div class="rounded-md px-6 py-5 corex-page-banner">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div>
                <h1 class="text-base font-bold leading-tight" style="color: var(--text-primary);">Leave Dashboard</h1>
                <p class="text-xs" style="color: var(--text-muted);">Track balances, applications and compliance at a glance.</p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                @permission('manage_leave')
                <a href="{{ route('payroll.leave.balances.index') }}" class="corex-btn-primary text-xs">View Balances</a>
                @endpermission
                @permission('approve_leave')
                <a href="{{ route('payroll.leave.applications.index') }}" class="corex-btn-outline text-xs">View Applications</a>
                @endpermission
                @permission('view_leave_reports')
                <a href="{{ route('payroll.leave.reports.register') }}" class="corex-btn-outline text-xs">View Reports</a>
                @endpermission
    ...
    @php
        $kpiTiles = [
            ...
            [
                ...
                'fg'    => 'var(--ds-amber)',
                'value_color' => $pendingApplications > 0 ? 'var(--ds-amber)' : 'var(--text-primary)',
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 xl:gap-4">
        ...
        <div class="pstat-v2 px-5 py-4 flex items-center justify-between gap-3">
            <div class="min-w-0">
                <div class="text-[1.625rem] font-bold leading-none tabular-nums" style="color:{{ $kpi['value_color'] }};">{{ $kpi['value'] }}</div>
            ...
            <span class="inline-flex items-center justify-center w-9 h-9 rounded-md flex-shrink-0"
                  style="background:{{ $kpi['bg'] }};color:{{ $kpi['fg'] }};border:1px solid color-mix(in srgb, {{ $kpi['fg'] }} 40%, transparent);">
