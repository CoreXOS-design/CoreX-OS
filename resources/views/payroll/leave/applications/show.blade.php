...
<div class="w-full space-y-5">
    <div class="rounded-md px-6 py-5 corex-page-banner">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div>
                <h1 class="text-base font-bold leading-tight" style="color: var(--text-primary);">Application {{ $application->application_number }}</h1>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                @include('layouts.partials.tour-header-launcher', ['variant' => 'surface'])
                <a href="{{ route('payroll.leave.applications.index') }}" class="corex-btn-outline text-xs shrink-0">&larr; Applications</a>
            </div>
        </div>
    </div>
        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="lg:w-1/3 space-y-4">
                ...
                <div class="p-4" style="background:var(--surface); border:1px solid var(--border); border-radius:6px;">
                    <h4 class="text-xs font-bold uppercase mb-2" style="color:var(--text-muted); letter-spacing:0.05em;">Applicant</h4>
                ...
                <div class="p-4" style="background:var(--surface); border:1px solid var(--border); border-radius:6px;">
                    ...
                    <dl class="space-y-1.5 text-xs">
                        ...
                        @if($application->is_half_day)
                            <div class="flex justify-between"><dt style="color:var(--text-muted);">Half Day</dt><dd style="color:var(--text-primary);">{{ ucfirst($application->half_day_period) }}</dd></div>
                        @endif
                        @if($application->reason)
                            <div><dt style="color:var(--text-muted);">Reason:</dt><dd class="mt-0.5" style="color:var(--text-primary);">{{ $application->reason }}</dd></div>
                        @endif
                    </dl>
                </div>
                ...
                @if($balanceBefore)
                <div class="p-4" style="background:var(--surface); border:1px solid var(--border); border-radius:6px;">
                    <h4 class="text-xs font-bold uppercase mb-2" style="color:var(--text-muted); letter-spacing:0.05em;">Balance Impact</h4>
                    <dl class="space-y-1.5 text-xs">
                        <div class="flex justify-between"><dt style="color:var(--text-muted);">Before</dt><dd class="font-semibold" style="color:var(--text-primary);">{{ number_format((float)$balanceBefore['available_days'], 2) }} days</dd></div>
                        <div class="flex justify-between"><dt style="color:var(--text-muted);">This Application</dt><dd class="font-semibold" style="color:var(--ds-crimson);">-{{ number_format($application->working_days_requested, 2) }} days</dd></div>
                        @php $afterBalance = bcsub($balanceBefore['available_days'], (string)$application->working_days_requested, 2); @endphp
                        <div class="flex justify-between"><dt style="color:var(--text-muted);">After (if approved)</dt><dd class="font-semibold" style="color:{{ (float)$afterBalance < 0 ? '#ef4444' : '#00d4aa' }};">{{ number_format((float)$afterBalance, 2) }} days</dd></div>
                    </dl>
                ...
                <div class="p-4" style="background:var(--surface); border:1px solid var(--border); border-radius:6px;">
                    <h4 class="text-xs font-bold uppercase mb-2" style="color:var(--text-muted); letter-spacing:0.05em;">Documents</h4>
                    @foreach($application->documents as $doc)
                        <p class="text-xs" style="color:var(--text-primary);">{{ $doc->original_name ?? $doc->file_name ?? 'Document' }}</p>
                    @endforeach
                </div>
                @endif
            </div>
            ...
            <div class="lg:w-2/3 space-y-4">
                ...
                <div class="p-4" style="background:{{ $sc[1] }}; border:1px solid {{ $sc[0] }}25; border-radius:6px;">
                    ...
                    @if($application->decision_reason)
                        <p class="text-xs mt-1" style="color:var(--text-primary);">Reason: {{ $application->decision_reason }}</p>
                    @endif
                    @if($application->cancellation_reason)
                        <p class="text-xs mt-1" style="color:var(--text-primary);">Cancellation: {{ $application->cancellation_reason }}</p>
                    @endif
                </div>

                {{-- Decision panel (pending only) --}}
                @if($application->isSubmitted())
                <div class="p-4" style="background:var(--surface); border:1px solid var(--border); border-radius:6px;" x-data="{ showReject: false }">
                    <h4 class="text-xs font-bold uppercase mb-3" style="color:var(--text-muted); letter-spacing:0.05em;">Decision</h4>
                    <div class="flex gap-3">
                        <form method="POST" action="{{ route('payroll.leave.applications.approve', $application) }}" class="inline">
                            @csrf
                            <button type="submit" class="corex-btn-primary text-sm" onclick="return confirm('Approve this leave application?')">Approve</button>
                        </form>
                    ...
                    <form method="POST" action="{{ route('payroll.leave.applications.reject', $application) }}" x-show="showReject" x-cloak class="mt-3 space-y-3">
                        @csrf
                        <div>
                            <label class="block text-xs font-semibold mb-1" style="color:var(--text-muted);">Reason for rejection (min 10 chars) <span class="text-red-500">*</span></label>
                            <textarea name="decision_reason" required minlength="10" rows="3" class="w-full px-3 py-2 text-sm focus:outline-none" style="background:var(--surface-2); border:1px solid var(--border); color:var(--text-primary); border-radius:6px;" placeholder="Explain why this application is being rejected..."></textarea>
                        ...
                        <div class="flex gap-2">
                            ...
                            <button type="button" @click="showReject = false" class="px-3 py-1.5 text-xs font-semibold" style="color:var(--text-secondary); border:1px solid var(--border); border-radius:6px; background:none; cursor:pointer;">Cancel</button>
                        </div>
                    </form>
                </div>
                @endif
                ...
                {{-- Conflict check --}}
                <div class="p-4" style="background:var(--surface); border:1px solid var(--border); border-radius:6px;">
                    <h4 class="text-xs font-bold uppercase mb-2" style="color:var(--text-muted); letter-spacing:0.05em;">Team Leave Conflicts</h4>
                    @if($conflicts->isEmpty())
                        <p class="text-xs" style="color:var(--text-muted);">No other staff on leave during this period.</p>
                    @else
                        <ul class="space-y-1">
                ...
                <div class="p-4" style="background:var(--surface); border:1px solid var(--border); border-radius:6px;">
                    <h4 class="text-xs font-bold uppercase mb-2" style="color:var(--text-muted); letter-spacing:0.05em;">Transactions</h4>
                    <div class="space-y-2">
                        @foreach($appTransactions as $txn)
                            <div class="flex items-start gap-3 py-1.5" style="border-bottom:1px solid var(--border);">
                                ...
                                <div>
                                    <p class="text-xs" style="color:var(--text-primary);">{{ $txn->description }} <span class="font-semibold" style="color:{{ (float)$txn->days_delta >= 0 ? '#00d4aa' : '#ef4444' }};">{{ $txn->days_delta > 0 ? '+' : '' }}{{ number_format((float)$txn->days_delta, 2) }} days</span></p>
                                    <p class="text-[10px]" style="color:var(--text-muted);">{{ $txn->createdBy->name ?? 'System' }} | {{ $txn->created_at?->format('d M Y H:i') }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
