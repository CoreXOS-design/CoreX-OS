        ...
        <div class="space-y-1.5 text-xs">
            @foreach([
                ...
                <div class="flex items-center gap-2">
                    @if($done)
                        <svg class="w-3.5 h-3.5" style="color:var(--brand-icon, #0ea5e9);" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                    @else
                        <svg class="w-3.5 h-3.5" style="color:var(--ds-amber, #f59e0b);" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v3.75m9-.75a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9 3.75h.008v.008H12v-.008Z"/></svg>
                    @endif
                    <span style="color:{{ $done ? 'var(--text-primary, #0f172a)' : 'var(--ds-amber, #f59e0b)' }};">{{ $label }}</span>
                </div>
            @endforeach
        </div>
        <p class="text-[10px] mt-2" style="color:var(--text-secondary, #94a3b8);">Progress: {{ number_format($takeOn->progressPercentage()) }}%</p>
    </div>
