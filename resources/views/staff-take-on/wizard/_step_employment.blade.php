    ...
    <div class="p-4" style="background:var(--surface-2, #f8fafc); border:1px solid var(--border, #e5e7eb); border-radius:6px;">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div><label class="block text-xs font-semibold mb-1" style="color:var(--text-secondary, #6b7280);">Working Pattern <span class="text-red-500">*</span></label>
                <select name="working_pattern" required class="w-full px-3 py-2 text-sm focus:outline-none" style="background:#fff; border:1px solid var(--border, #e5e7eb); color:var(--text-primary, #0f172a); border-radius:6px;" onchange="document.getElementById('wdpw').value = this.value === 'monday_to_saturday' ? 6 : 5">
            ...
            <div><label class="block text-xs font-semibold mb-1" style="color:var(--text-secondary, #6b7280);">Hours per Day <span class="text-red-500">*</span></label>
                <input type="number" name="hours_per_day" required step="0.5" min="1" max="24" value="{{ old('hours_per_day', $pe?->hours_per_day ?? 8) }}" class="w-32 px-3 py-2 text-sm focus:outline-none" style="background:#fff; border:1px solid var(--border, #e5e7eb); color:var(--text-primary, #0f172a); border-radius:6px;"></div>
            <div><label class="block text-xs font-semibold mb-1" style="color:var(--text-secondary, #6b7280);">Pay Day of Month <span class="text-red-500">*</span></label>
                <input type="number" name="pay_day_of_month" required min="1" max="31" value="{{ old('pay_day_of_month', $pe?->pay_day_of_month ?? 25) }}" class="w-32 px-3 py-2 text-sm focus:outline-none" style="background:#fff; border:1px solid var(--border, #e5e7eb); color:var(--text-primary, #0f172a); border-radius:6px;"></div>
            <div><label class="block text-xs font-semibold mb-1" style="color:var(--text-secondary, #6b7280);">Daily Rate Basis <span class="text-red-500">*</span></label>
                <select name="daily_rate_basis" required class="w-full px-3 py-2 text-sm focus:outline-none" style="background:#fff; border:1px solid var(--border, #e5e7eb); color:var(--text-primary, #0f172a); border-radius:6px;">
