...
<form method="POST" action="{{ route('staff-take-on.save-step', [$takeOn, 'leave']) }}">
    ...
    <div class="space-y-4">
        <div class="p-3 text-xs rounded-md" style="background:color-mix(in srgb, var(--brand-icon) 5%, transparent); border:1px solid color-mix(in srgb, var(--brand-icon) 15%, transparent); color:var(--text-muted);">
            ...
            @foreach($leaveTypes as $type)
                ...
                <div class="p-4 rounded-md" style="background:var(--surface); border:1px solid var(--border);">
                    <div class="flex items-center justify-between mb-2">
                        <h4 class="text-sm font-semibold" style="color:var(--text-primary);">{{ $type->label }}</h4>
                        @if($type->is_system)
                            <span class="px-1.5 py-0.5 text-[10px] font-bold uppercase rounded-md" style="background:color-mix(in srgb, var(--text-muted) 18%, transparent); color:var(--text-muted);">System</span>
                        @endif
                    </div>

                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs mb-3">
                        <div>
                            <span style="color:var(--text-muted);">Cycle:</span><br>
                            <strong>{{ $cycleStart->format('d M Y') }} — {{ $cycleEnd->format('d M Y') }}</strong>
                        </div>
                        <div>
                            <span style="color:var(--text-muted);">Entitlement:</span><br>
                        ...
                        <div>
                            <span style="color:var(--text-muted);">Cycle length:</span><br>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold mb-1" style="color:var(--text-muted);">Days already taken this cycle</label>
                            <input type="number" name="taken_{{ $type->id }}" step="0.5" min="0" value="0"
                                   class="w-32 px-3 py-2 text-sm rounded-md focus:outline-none" style="background:var(--surface-2); border:1px solid var(--border); color:var(--text-primary);">
                        </div>
                        @if($type->carries_over_to_next_cycle)
                        <div>
                            <label class="block text-xs font-semibold mb-1" style="color:var(--text-muted);">Carryover from previous cycle</label>
