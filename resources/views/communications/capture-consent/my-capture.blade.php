...
<div class="w-full space-y-5" x-data="captureConsent()">
    ...
    <div class="rounded-md px-4 py-3 text-sm flex items-start gap-3"
        ...
        <div class="flex-1">
            <strong>{{ number_format($pendingCount) }}</strong> matched {{ \Illuminate\Support\Str::plural('contact', $pendingCount) }} awaiting your decision — choose whether to capture your WhatsApp with them. Until you decide, their message bodies are not captured.
        </div>
    </div>
    @endif
                ...
                @forelse($rows as $row)
                    ...
                    <tr style="border-top:1px solid var(--border); {{ $row->status==='pending' ? 'background:color-mix(in srgb, var(--ds-amber,#f59e0b) 7%, transparent);' : '' }}">
                        <td class="px-4 py-3">
                            ...
                            <span class="ml-2 inline-block text-[11px] font-semibold px-2 py-0.5 rounded-md whitespace-nowrap" style="background:color-mix(in srgb, var(--brand-icon,#0ea5e9) 12%, transparent); color:var(--brand-icon,#0ea5e9);" title="{{ $row->admin_flag_note }}">Manager flagged as business</span>
                            @endif
                        </td>
                        <td class="px-4 py-3">
                            @php
                                $map = [
                                    ...
                                    'opted_in'  => ['Capturing',         'ds-badge-success'],
                                    'opted_out' => ['Not capturing',     'ds-badge-default'],
                                ];
