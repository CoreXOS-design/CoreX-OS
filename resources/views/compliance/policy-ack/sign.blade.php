...
<div class="-m-4 lg:-m-6" x-data="policySign()" x-init="init()">
    <x-page-header :title="'Complete: ' . $policyModel->name" :back-route="route('agent.portal') . '#compliance'" back-label="My Portal" :flush="true" />
    ...
    <div class="p-4 lg:p-6">
        <div class="max-w-2xl mx-auto space-y-5">
            ...
            <div class="px-4 py-3 text-sm" style="background:color-mix(in srgb, var(--ds-amber) 8%, transparent); border:1px solid rgba(234,179,8,0.25); border-radius:6px; color:#a16207;">
                ...
                For a more complete acknowledgement record, consider adding it in
                <a href="{{ route('agent.portal') }}#profile" style="text-decoration:underline; color:#92400e;">My Portal &mdash; Profile</a> before signing.
            ...
            <div class="bg-white border p-6" style="border-color:var(--border, #e5e7eb); border-radius:6px;">
                ...
                <div class="space-y-3 text-sm" style="color:#334155; line-height:1.7;">
                    ...
                    <div class="prose prose-sm max-w-none" style="color:#334155;">
                        {!! $declarationText !!}
                    </div>
                </div>
                <div class="mt-4 pt-3 text-xs" style="border-top:1px solid var(--border, #e5e7eb); color:#94a3b8;">
                    {{ $policyModel->name }} v{{ $version->version_number }} | {{ $agency->name }} | {{ $ack->sections_acknowledged_count }} of {{ $ack->sections_total_count }} sections acknowledged
                </div>
            </div>
            ...
            <div>
                <label class="flex items-start gap-3 cursor-pointer p-3 -m-3">
                    ...
                    <span class="text-xs" style="color:var(--text-primary, #1f2937); line-height:1.5;">
                        I confirm that I have read and understood the {{ $policyModel->name }} in full, and acknowledge my obligations under it.
                    </span>
                </label>
            </div>
            ...
            <div class="flex items-center justify-between gap-4 pb-24">
                ...
                <button type="button"
                        ...
                        :class="canSubmit && !isSubmitting ? 'hover:opacity-90' : 'cursor-not-allowed'"
...
function policySign() {
    return {
        ...
        async submitSignature() {
            ...
            try {
                const res = await fetch('{{ route("policy.ack.submit", $policyKey) }}', {
                    method: 'POST',
                ...
                window.location.href = res.url || '{{ route("policy.ack.receipt", [$policyKey, $ack]) }}';
            } catch (e) {
