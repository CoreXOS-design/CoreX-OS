            ...
            <form method="POST" action="{{ $isEdit ? route('compliance.comm-mailboxes.update', $mailbox) : route('compliance.comm-mailboxes.store') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="password" name="password" autocomplete="new-password" {{ $isEdit ? '' : 'required' }}
                           class="w-full px-3 py-2 text-sm border" style="border-color:var(--border, #e5e7eb); border-radius:6px;">
                    <p class="text-xs mt-1" style="color:#94a3b8;">Stored encrypted at rest. Never displayed back.</p>
                ...
                <div class="flex items-center gap-3 pt-2">
                    ...
                    <a href="{{ route('compliance.comm-mailboxes.index') }}" class="text-sm" style="color:#6b7280;">Cancel</a>
                </div>
            </form>
        </div>
    </div>
