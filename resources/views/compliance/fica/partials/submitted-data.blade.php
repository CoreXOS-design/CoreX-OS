...
<div class="rounded-md p-5" style="background:var(--surface); border:1px solid var(--border);">
    ...
    @forelse($submission->documents as $doc)
        <div class="flex items-center justify-between py-2" style="{{ !$loop->last ? 'border-bottom:1px solid var(--border);' : '' }}">
            <div>
                <span class="text-xs font-semibold uppercase" style="color:var(--text-secondary);">{{ $doc->document_type_label }}</span>
                <p class="text-sm" style="color:var(--text-primary);">{{ $doc->file_name }}</p>
                <p class="text-xs" style="color:var(--text-muted);">{{ number_format($doc->file_size / 1024) }} KB</p>
            </div>
            <a href="{{ Storage::url($doc->file_path) }}" target="_blank" class="text-xs font-medium" style="color:var(--brand-icon,#0ea5e9);">View</a>
        </div>
    @empty
