...
<div class="p-6 lg:p-8" x-data="coReview()">
    <div class="mb-6">
        <a href="{{ route('compliance.fica.show', $submission) }}" class="text-sm mb-2 inline-block" style="color:var(--text-muted)">&larr; Back to Submission</a>
        <h1 class="text-2xl font-bold" style="color:var(--text-primary)">Compliance Officer Review</h1>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div class="space-y-4">
            ...
            <div class="border p-5" style="background:var(--surface); border-color:var(--border)">
                <h3 class="text-sm font-bold mb-3 pb-2 border-b border-indigo-500" style="color:var(--text-primary)">Agent Review</h3>
                <dl class="space-y-2 text-sm">
                    ...
                    <div><dt class="text-xs" style="color:var(--text-faint)">Verification Method</dt><dd>@foreach($submission->verification_method as $m)<span class="inline-block px-1.5 py-0.5 text-xs mr-1 mb-1" style="background:var(--surface-2); color:var(--text-secondary)">{{ str_replace('_', ' ', ucfirst($m)) }}</span>@endforeach</dd></div>
                    @endif
                </dl>
            </div>
            ...
            @if(!empty($agentData))
            <div class="border p-5" style="background:var(--surface); border-color:var(--border)">
                <h3 class="text-sm font-bold mb-3 pb-2 border-b border-indigo-500" style="color:var(--text-primary)">Agent Checklist</h3>
                <dl class="space-y-2 text-sm">
            ...
            <div class="border p-5" style="background:var(--surface); border-color:var(--border)">
                <h3 class="text-sm font-bold mb-3 pb-2 border-b border-indigo-500" style="color:var(--text-primary)">Agent Notes</h3>
        ...
        <div class="space-y-4">
                ...
                <div class="space-y-3 text-sm">
                    ...
                    <div>
                        ...
                        <div x-show="coChecklist.suspicious === 'yes'" x-cloak class="mt-1">
                            <textarea x-model="coChecklist.suspicious_details" rows="2" class="w-full px-2 py-1 border text-xs focus:outline-none focus:border-emerald-500" style="border-color:var(--border)" placeholder="Details..."></textarea>
                        </div>
                        @endif
                    </div>
                    @endforeach
            ...
            <form method="POST" action="{{ route('compliance.fica.compliance-approve', $submission) }}" @submit.prevent="submitApproval">
                ...
                <div class="border p-5 space-y-4" style="background:var(--surface); border-color:var(--border)">
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ auth()->user()->name }}" class="w-full px-3 py-2 border text-sm" style="border-color:var(--border); background:var(--surface-2)" readonly>
                    </div>

                    <div>
                        <label class="block text-xs font-semibold mb-1" style="color:var(--text-secondary)">Notes</label>
                        <textarea name="co_notes" rows="3" class="w-full px-3 py-2 border text-sm focus:outline-none focus:border-emerald-500" style="border-color:var(--border)" placeholder="Optional compliance notes..."></textarea>
                    </div>

                    <div>
                        <label class="block text-xs font-semibold mb-1" style="color:var(--text-secondary)">Your Signature *</label>
                        <div style="position: relative;">
                            <canvas x-ref="coSignatureCanvas" width="400" height="120" style="width: 100%; border: 1px solid var(--border); background: var(--surface); touch-action: none; cursor: crosshair;"></canvas>
                            <button type="button" @click="clearSignature()" style="position: absolute; top: 0.25rem; right: 0.25rem; font-size: 0.7rem; color: var(--text-muted); background: var(--surface); border: 1px solid var(--border); padding: 0.15rem 0.4rem; cursor: pointer;">Clear</button>
                        </div>
            ...
            <form method="POST" action="{{ route('compliance.fica.compliance-reject', $submission) }}">
                ...
                <div class="border p-5" style="background:var(--surface); border-color:var(--border)">
                    ...
                    <textarea name="reviewer_notes" rows="2" class="w-full px-3 py-2 border text-sm focus:outline-none focus:border-amber-500 mb-3" style="border-color:var(--border)" placeholder="Reason for returning..." required></textarea>
            ...
            <form method="POST" action="{{ route('compliance.fica.compliance-reject', $submission) }}">
                ...
                <div class="border p-5" style="background:var(--surface); border-color:var(--border)">
                    ...
                    <textarea name="reviewer_notes" rows="2" class="w-full px-3 py-2 border text-sm focus:outline-none focus:border-red-500 mb-3" style="border-color:var(--border)" placeholder="Reason for rejection..." required></textarea>
            ...
            @if($submission->status === 'referred_to_co' && ($viewerOwnsReferralStation ?? false))
                <form method="POST" action="{{ route('compliance.fica.return-to-referrer', $submission) }}">
                    ...
                    <div class="border p-5" style="background:var(--surface); border-color:var(--border)">
                        ...
                        <textarea name="reviewer_notes" rows="2" required minlength="3" maxlength="2000" class="w-full px-3 py-2 border text-sm focus:outline-none focus:border-amber-500 mb-3" style="border-color:var(--border)" placeholder="Comments for the referrer..."></textarea>
