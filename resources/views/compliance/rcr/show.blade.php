...
<div style="max-width:1280px;margin:0 auto;padding:0 20px;" x-data="rcrShow({
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3" style="margin-bottom:14px;">
        <div>
            <h1 class="text-base font-bold leading-tight" style="color: var(--text-primary);">{{ $submission->questionnaire->title }}</h1>
            <p class="text-xs" style="color: var(--text-muted); margin:4px 0 0 0;">
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            @if($isEditable)
                <form method="POST" action="{{ route('corex.compliance.rcr.auto-populate', $submission->id) }}" style="display:inline;">
                    @csrf
                    <button type="submit" class="corex-btn-outline text-xs">Auto-populate New Data</button>
                </form>
            @endif
            <a href="{{ route('corex.compliance.rcr.export', [$submission->id, 'pdf']) }}" class="corex-btn-outline text-xs" style="text-decoration:none;">Export PDF</a>
            <a href="{{ route('corex.compliance.rcr.export', [$submission->id, 'csv']) }}" class="corex-btn-outline text-xs" style="text-decoration:none;">CSV</a>
            <a href="{{ route('corex.compliance.rcr.export', [$submission->id, 'json']) }}" class="corex-btn-outline text-xs" style="text-decoration:none;">JSON</a>
            <a href="{{ route('corex.compliance.rcr.index') }}" class="corex-btn-outline text-xs" style="text-decoration:none;">← Back to RCR list</a>
        </div>
    </div>

    @if (session('status'))
        <div style="background:color-mix(in srgb, var(--ds-green, #059669) 10%, transparent);color:var(--text-primary);border:1px solid color-mix(in srgb, var(--ds-green, #059669) 30%, transparent);border-radius:6px;padding:10px 14px;font-size:0.8125rem;margin-bottom:14px;">{{ session('status') }}</div>
    @endif
    @if ($errors->any())
        <div style="background:color-mix(in srgb, var(--ds-crimson, #c41e3a) 10%, transparent);color:var(--text-primary);border:1px solid color-mix(in srgb, var(--ds-crimson, #c41e3a) 30%, transparent);border-radius:6px;padding:10px 14px;font-size:0.8125rem;margin-bottom:14px;">{{ $errors->first() }}</div>
    @endif
    ...
    <div style="background:var(--surface);border:1px solid var(--border);border-radius:6px;padding:14px;margin-bottom:14px;">
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:14px;">
            <div>
                ...
                <div style="font-size:1.25rem;font-weight:700;color:var(--text-primary);">{{ $stats['total'] }}</div>
            </div>
            <div>
                <div style="font-size:0.625rem;color:var(--text-muted);text-transform:uppercase;">Auto-filled</div>
                <div style="font-size:1.25rem;font-weight:700;color:var(--brand-icon, #0ea5e9);">{{ $stats['auto_filled'] }}</div>
            </div>
            <div>
            ...
            <div>
                ...
                <div style="font-size:1.25rem;font-weight:700;color:var(--text-primary);">{{ $stats['progress_pct'] }}%</div>
            </div>
        </div>
        <div style="margin-top:10px;background:var(--surface-2);height:8px;border-radius:4px;overflow:hidden;">
            <div style="background:var(--brand-icon, #0ea5e9);height:100%;width:{{ $stats['progress_pct'] }}%;"></div>
        </div>
    </div>
                    ...
                    @foreach($section->questions as $question)
                        ...
                        <div style="padding:12px 0;border-top:{{ $loop->first ? '0' : '1px solid var(--border)' }};" x-data="rcrAnswer({
                            ...
                            @if($a && $a->is_auto_populated && !$a->manually_edited)
                                ...
                                <div style="background:color-mix(in srgb, var(--brand-icon, #0ea5e9) 10%, transparent);color:var(--text-primary);border:1px solid color-mix(in srgb, var(--brand-icon, #0ea5e9) 30%, transparent);border-radius:4px;padding:8px 10px;margin-bottom:8px;font-size:0.75rem;">
                                    <strong>Auto-filled from {{ $sourceData['source'] ?? 'CoreX' }}</strong>
                                    @if(!empty($sourceData['pulled_at']))
                                        <span style="color:var(--text-muted);font-size:0.6875rem;"> · pulled {{ \Carbon\Carbon::parse($sourceData['pulled_at'])->diffForHumans() }}</span>
                                    @endif
                            ...
                            @if($isEditable)
                                <textarea x-model="value" @input.debounce.800ms="save()" rows="2" maxlength="65000"
                                          style="width:100%;padding:8px 10px;background:var(--surface-2);border:1px solid var(--border);border-radius:4px;font-size:0.8125rem;font-family:inherit;color:var(--text-primary);"
                                          placeholder="Type your answer…"></textarea>
                                <div style="display:flex;justify-content:flex-end;gap:6px;margin-top:6px;">
                                    <button type="button" @click="markAnswered" :disabled="!value"
                                            style="font-size:0.6875rem;padding:5px 12px;background:var(--brand-button, #0ea5e9);color:#fff;border:0;border-radius:6px;cursor:pointer;font-weight:600;">
                                        Mark as answered
                                    </button>
                                </div>
                            @else
                ...
                <div style="background:var(--surface);border:1px solid var(--border);border-radius:6px;padding:14px;margin-bottom:30px;">
                    ...
                    <form method="POST" action="{{ route('corex.compliance.rcr.submit', $submission->id) }}">
                        ...
                        <input type="text" name="submitted_to_platform_reference" maxlength="200"
                               style="width:320px;padding:6px 10px;background:var(--surface-2);border:1px solid var(--border);border-radius:4px;font-size:0.8125rem;margin-bottom:10px;color:var(--text-primary);">
