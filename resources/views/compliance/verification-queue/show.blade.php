...
<div class="w-full space-y-5" x-data="{ showRejectModal: false, rejectReason: '' }">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="lg:col-span-2" style="background:var(--surface); border:1px solid var(--border); border-radius:6px; overflow:hidden;">
            <div class="px-5 py-3" style="border-bottom:1px solid var(--border);">
                <h3 class="text-sm font-bold" style="color:var(--text-primary);">Document Preview</h3>
            </div>
            ...
            <div style="padding:16px; min-height:400px;">
                ...
                <iframe src="{{ $publicUrl }}" style="width:100%; height:600px; border:1px solid var(--border); border-radius:6px;"></iframe>
                @elseif(in_array($extension, ['jpg', 'jpeg', 'png', 'gif', 'webp']))
                <div style="text-align:center;">
                    <img src="{{ $publicUrl }}" alt="{{ $document->file_name }}"
                         style="max-width:100%; max-height:600px; border-radius:6px; border:1px solid var(--border);">
                </div>
                @else
        ...
        <div class="space-y-4">
            ...
            <div style="background:var(--surface); border:1px solid var(--border); border-radius:6px; padding:16px 20px;">
                ...
                <div class="space-y-2">
                    <div class="flex justify-between">
                        ...
                        <span style="font-size:0.7rem; font-weight:600; padding:2px 8px; border-radius:6px; background:color-mix(in srgb, var(--brand-icon) 8%, transparent); color:var(--brand-icon);">{{ \App\Models\UserDocument::$documentTypeLabels[$document->document_type] ?? $document->document_type }}</span>
                    </div>
                    <div class="flex justify-between">
            ...
            <div style="background:var(--surface); border:1px solid var(--border); border-radius:6px; padding:16px 20px;">
                ...
                <div class="flex items-center gap-3 mb-3">
                    <span style="font-size:0.75rem; font-weight:700; padding:3px 12px; border-radius:6px; background:{{ $sc['bg'] }}; color:{{ $sc['color'] }};">{{ $sc['text'] }}</span>
                </div>
    ...
    <div x-show="showRejectModal" x-cloak x-transition
        ...
        <div @click.outside="showRejectModal = false"
             style="background:var(--surface); border:1px solid var(--border); border-radius:6px; padding:24px; width:100%; max-width:480px;">
            <h3 style="font-size:0.9rem; font-weight:700; color:var(--text-primary); margin:0 0 6px;">Reject Document</h3>
            ...
            <form method="POST" action="{{ route('compliance.verification.reject', $document) }}">
                ...
                <textarea name="rejected_reason" x-model="rejectReason" rows="4" required maxlength="1000"
                          ...
                          style="width:100%; border-radius:6px; border:1px solid var(--border); background:var(--surface-2); color:var(--text-primary); padding:10px 14px; font-size:0.8rem; box-sizing:border-box; resize:vertical; transition:border-color 200ms;"
                          onfocus="this.style.borderColor='var(--ds-crimson)'" onblur="this.style.borderColor='var(--border)'"></textarea>
