...
<div class="w-full space-y-5" x-data="dealCapture()">
    ...
    <div class="rounded-md px-6 py-5" style="background: var(--brand-default, #0b2a4a);">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div>
                ...
                <p class="text-sm text-white/60">Capture the property, parties and commission on one page — then generate the deal pipeline.</p>
            </div>
            <div class="flex items-center gap-2 flex-wrap">
                <a href="{{ route('deals-v2.create-wizard') }}"
                ...
                <button type="submit" form="dealForm" class="corex-btn-primary">Create Deal</button>
            </div>
        </div>
    </div>
                    ...
                    <div class="relative">
                        ...
                        <div x-show="propertyResults.length" class="absolute z-20 mt-1 w-full rounded-md shadow-lg max-h-60 overflow-auto"
                            ...
                            <template x-for="p in propertyResults" :key="p.id">
                                <button type="button" @click="selectProperty(p)" class="deal-result-btn block w-full text-left px-3 py-2 text-sm transition-colors"
                    ...
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <label class="inline-flex items-center gap-1.5 text-xs cursor-pointer" style="color: var(--text-secondary);">
                            <input type="checkbox" name="{{ $side }}_external" value="1" id="{{ $side }}_external_cb"
                                   {{ old($side . '_external') ? 'checked' : '' }} class="rounded" style="accent-color: var(--brand-button, #0ea5e9);"
                    ...
                    <div class="relative">
                        ...
                        <div x-show="sellerResults.length" class="absolute z-20 mt-1 w-full rounded-md shadow-lg max-h-52 overflow-auto" style="background: var(--surface); border: 1px solid var(--border);">
                            <template x-for="c in sellerResults" :key="c.id">
                                <button type="button" @click="addContact(c, 'seller')" class="deal-result-btn block w-full text-left px-3 py-2 text-sm transition-colors" style="color: var(--text-primary); border-bottom: 1px solid var(--border);">
                        ...
                        <template x-for="c in contacts.filter(x => x.role === 'seller' || x.role === 'co_seller')" :key="c.contact_id + c.role">
                            <div class="flex items-center justify-between rounded px-2.5 py-1.5 text-sm" style="background: var(--surface-2);">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <button type="button" @click="removeContact(c)" class="text-xs" style="color: var(--text-muted);">✕</button>
                                </div>
                            </div>
                        </template>
                    </div>
                    ...
                    <div class="relative">
                        ...
                        <div x-show="buyerResults.length" class="absolute z-20 mt-1 w-full rounded-md shadow-lg max-h-52 overflow-auto" style="background: var(--surface); border: 1px solid var(--border);">
                            <template x-for="c in buyerResults" :key="c.id">
                                <button type="button" @click="addContact(c, 'buyer')" class="deal-result-btn block w-full text-left px-3 py-2 text-sm transition-colors" style="color: var(--text-primary); border-bottom: 1px solid var(--border);">
                                    <span x-text="c.name"></span> <span class="text-xs" style="color: var(--text-muted);" x-text="c.phone || c.email || ''"></span>
                                </button>
                            </template>
                        </div>
