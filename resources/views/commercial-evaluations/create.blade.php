...
<div class="max-w-3xl mx-auto" x-data="{ propertyType: '{{ old('property_type', '') }}' }">
    ...
    <x-sticky-action-bar>
        <x-slot name="left">
            <a href="{{ route('commercial-evaluations.index') }}" class="inline-flex items-center gap-1 text-sm transition-colors" style="color: var(--text-secondary);">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                Back
            </a>
        </x-slot>
        <x-slot name="center">
            <h2 class="text-sm font-semibold truncate" style="color: var(--text-primary);">New Commercial Market Evaluation</h2>
        </x-slot>
    </x-sticky-action-bar>
    ...
    <div class="ds-status-card mb-6" style="border-left-color: var(--ds-cyan);">
        <div class="px-5 py-4">
            <h3 class="text-sm font-semibold mb-3" style="color: var(--text-primary);">Step 1: Select Property Type</h3>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <label class="relative cursor-pointer">
                    ...
                    <div class="border-2 rounded-md p-4 text-center transition-all peer-checked:border-[var(--brand-icon)] peer-checked:bg-[color-mix(in_srgb,var(--brand-icon)_12%,transparent)] hover:border-[var(--border)]"
                        ...
                        <span class="block text-[10px] mt-0.5" style="color: var(--text-muted);">Retail / Office</span>
                    </div>
                </label>
                ...
                <label class="relative cursor-pointer">
                    ...
                    <div class="border-2 rounded-md p-4 text-center transition-all peer-checked:border-amber-500 peer-checked:bg-amber-50 hover:border-[var(--border)]"
                         :class="propertyType === 'industrial' ? 'border-amber-500 bg-amber-50' : 'border-[var(--border)]'">
                        ...
                        <span class="text-xs font-semibold" style="color: var(--text-primary);">Industrial</span>
                        <span class="block text-[10px] mt-0.5" style="color: var(--text-muted);">Warehouse / Factory</span>
                    </div>
                </label>
                ...
                <label class="relative cursor-pointer">
                    ...
                    <div class="border-2 rounded-md p-4 text-center transition-all peer-checked:border-purple-500 peer-checked:bg-purple-50 hover:border-[var(--border)]"
                         :class="propertyType === 'hospitality' ? 'border-purple-500 bg-purple-50' : 'border-[var(--border)]'">
                        ...
                        <span class="text-xs font-semibold" style="color: var(--text-primary);">Hospitality</span>
                        <span class="block text-[10px] mt-0.5" style="color: var(--text-muted);">B&B / Guest House</span>
                    </div>
                </label>
                ...
                <label class="relative cursor-pointer">
                    ...
                    <div class="border-2 rounded-md p-4 text-center transition-all peer-checked:border-green-500 peer-checked:bg-green-50 hover:border-[var(--border)]"
                         :class="propertyType === 'agricultural' ? 'border-green-500 bg-green-50' : 'border-[var(--border)]'">
                        ...
                        <span class="text-xs font-semibold" style="color: var(--text-primary);">Agricultural</span>
                        <span class="block text-[10px] mt-0.5" style="color: var(--text-muted);">Farm / Smallholding</span>
                    </div>
                </label>
            </div>
        </div>
    ...
    <div x-show="propertyType !== ''" x-transition class="ds-status-card" style="border-left-color: var(--ds-cyan);">
        <form method="POST" action="{{ route('commercial-evaluations.store') }}">
            ...
            <div class="px-5 py-4 space-y-5">
                <h3 class="text-sm font-semibold" style="color: var(--text-primary);">Step 2: Property Details</h3>
            ...
            <div class="px-5 py-4 flex items-center gap-3" style="border-top: 1px solid var(--border);">
                ...
                <a href="{{ route('commercial-evaluations.index') }}" class="px-4 py-2 text-sm transition-colors" style="color: var(--text-secondary);">
                    Cancel
                </a>
            </div>
        </form>
