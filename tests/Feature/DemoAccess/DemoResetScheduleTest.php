...
class DemoResetScheduleTest extends TestCase
{
    ...
    /**
     * A failed backup PREVENTS the wipe — it is not absorbed and logged.
     *
     * Converged at the QA2 → Staging merge: Staging's demo:refresh never destroyed
     * anything it had not first dumped, and demo:reset inherited that. A demo
     * rebuild that wipes with nowhere to put the copy is unrecoverable, so the
     * backup is a precondition of the wipe, not a courtesy alongside it.
     *
     * Proven on a DEMO instance (the guard above is passed), so the ONLY thing
     * standing between this command and migrate:fresh is the backup.
     */
    public function test_a_failed_backup_prevents_the_wipe(): void
    {
        config()->set('corex.instance.role', 'demo');
        $this->assertTrue(Instance::isDemo(), 'Guard must be passed, or this proves nothing.');

        // Point the backup at a path that cannot be created: a directory whose
        // parent is a FILE. ensureDirectoryExists/fopen fail → backup() is false.
        $file = tempnam(sys_get_temp_dir(), 'demo-backup-block');
        config()->set('demo.refresh.backup_path', $file . '/nope');

        $this->artisan('demo:reset')
            ->expectsOutputToContain('REFUSED')
            ->assertExitCode(1);

        // The database is INTACT. Had the wipe run, migrate:fresh would have
        // dropped every table out from under RefreshDatabase.
        $this->assertTrue(\Illuminate\Support\Facades\Schema::hasTable('demo_access_grants'));
        $this->assertTrue(\Illuminate\Support\Facades\Schema::hasTable('properties'));
